...
Far inferenza statistica significa dedurre informazioni sul \textcolor{red}{fenomeno di interesse} sulla base di informazioni raccolte su un campione $C \subseteq P$. \\ \\
Come formo il campione dalla popolazione?
\begin{itemize}
	\item \textbf{Campione da popolazioni finite:} La popolazione esiste già prima dell'osservazione. Per formare il campione seleziono in modo casuale.
	\item \textbf{Campione da variabili aleatorie:} La popolazione non esiste, formo il campione con la creazione di dati attraverso l'esperimento.
\end{itemize}
\begin{esempio}[Esempio \textcolor{orange}{campionamento da popolazioni finite}: Studio Salariale]
	Voglio conoscere il salario annuo iniziale di laureati di 1º livello in informatica nelle aziende italiane in un dato anno:
	\begin{itemize}
		\item Fenomeno di interesse: salario annuo (parametro incognito)
		\item Campione: sottoinsieme di informatici di 1º livello occupati in aziende italiane
	\end{itemize}
	Attraverso la media aritmetica dei valori osservati (vari stipendi annui) posso stimare lo stipendio medio incognito di tutta la popolazione.
\end{esempio}
\begin{esempio}[Esempio \textcolor{orange}{campionamento da variabili aleatorie}: Studio Componenti Elettronici]
	Voglio conoscere la durata di vita di componenti elettronici prodotti da un'azienda italiana:
	\begin{itemize}
		\item Fenomeno di interesse: duarata di vita di un componente (parametro incognito)
		\item Campione: un campione di componenti viene messo in funzione e viene registrato il tempo che intercorre fino alla sua rottura
	\end{itemize}
	Attraverso la media aritmetica dei valori creati (vari tempi di vita) posso stimare la durata di vita media incognita di tutta la popolazione.
\end{esempio}
In entrambi i metodi di campionamento, i dati campionari sono generati da un meccanismo aleatorio (il primo li sceglie in modo casuale mentre il secondo li crea) che chiamo
\textbf{osservazioni}.
\begin{align*}
	\text{ogni osservazione } x \text{ è la realizzazione della variabile aletoria } X
\end{align*}
dove \textcolor{red}{$X$ è il fenomeno di interesse} che ha nella popolazione una sua distribuzione.
\begin{esempio}
	\begin{itemize}
		\item Studio Salariale: la variabile aleatoria è $X = $ (stipendio iniziale annuo)
		      e dalla lista estraggo:
		      \begin{itemize}
			      \item prima estrazione: $x_1 = 28.000$ \euro \, (Alice)
			      \item seconda estrazione: $x_2 = 40.000$ \euro \, (Marco)
			      \item $\cdots$
		      \end{itemize}
		\item Studio Componenti Elettronici: la variabile aleatoria è $X = $ (tempo di vita di un componente) accendo i componenti e registro i minuti:
		      \begin{itemize}
			      \item primo a spengersi: $x_1 = 3000$ minuti
			      \item secondo a spengersi: $x_2 = 3500$ minuti
			      \item $\cdots$
		      \end{itemize}
	\end{itemize}
\end{esempio}
L'obiettivo è individuare il valore del parametro ignoto attraverso i dati campionari: \\
si effettuano $n$ prove ($n$ lanci di moneta, $n$ estrazioni da una lista, attivazione di $n$ componenti elettronici, ...)
e si utilizzano i dati osservati per ottenere il parametro ignoto.
\begin{theorem}{Campione aletorio e Campione osservato}
	S\begin{itemize}
		\item \textcolor{red}{Campione aleatorio} di numerosità $n$ (prova $i-$esima):
		      \[
			      \vec{X} = \left(X_1,X_2, \cdots, X_n \right)
		      \]
		\item \textcolor{red}{Campione osservato} $x_i$ è il risultato della $i-$esima variabile aleatoria:
		      \[
			      X_i = x_i
		      \]
	\end{itemize}
...
\begin{esempio}
	\begin{itemize}
		\item Studio salariale: estraggo 3 persone dalla lista
		      \begin{align*}
			      X_1 = 40.000 \text{\euro} \\
			      X_2 = 50.000 \text{\euro} \\
			      X_3 = 60.000 \text{\euro}
		      \end{align*}
		\item Studio Componenti Elettronici: attivo e studio la durata di vita di 4 componenti
		      \begin{align*}
			      X_1 = 500 \text{ minuti}  \\
			      X_2 = 3000 \text{ minuti} \\
			      X_3 = 100 \text{ minuti}  \\
	...
	\end{itemize}
\end{esempio}
\begin{theorem}{Modello Statistico}
	SFamiglia parametrica di leggi di probabilità della variabile aletoria multipla $\vec{X} = \left(X_1, \cdots, X_n \right)$
	\begin{align*}
		\mathcal{F}_{\vec{X}} = \left\{ p_{\vec{X}}(\cdot, \theta); \, \theta \in \Theta \right\} \quad \text{oppure} \quad  \mathcal{F}_{\vec{X}} = \left\{ f_{\vec{X}}(\cdot; \theta); \, \theta \in \Theta \right\}
	\end{align*}
\end{theorem}
\begin{theorem}{Campione Casuale}
	SUn \textcolor{red}{campione casuale $\vec{X}$} di ampiezza $n$ è composto da $n$ variabili aleatorie $X_1, \cdots, X_n$ i.i.d quindi la probabilità (funzione di massa di probabilita $p_{\vec{X}}$/
	funzione di densità di probabilità $f_{\vec{X}}$) di osservare un generico campione $\vec{x}=(x_1, \cdots, x_n)$ è:
	\begin{align*}
		p_{\vec{X}}(\vec{x};\theta) = p_{\vec{X}}(x_1, \cdots, x_n;\theta) = \prod_{i=1}^{n}p_{\vec{X}}(\vec{x_i};\theta) \quad \quad \text{(caso discreto)} \\
		f_{\vec{X}}(\vec{x};\theta) = f_{\vec{X}}(x_1, \cdots, x_n;\theta) = \prod_{i=1}^{n}f_{\vec{X}}(\vec{x_i};\theta) \quad \quad \text{(caso continuo)}
	\end{align*}
\end{theorem}
\begin{esempio}
	Nel caso dei componenti elettronici
	\begin{align*}
		 & X_1 = 500 \quad \quad \text{il 1º dura 500 minuti}   \\
		 & X_2 = 3000 \quad \quad \text{il 2º dura 3000 minuti} \\
		 & X_3 = 100 \quad \quad \text{il 3º dura 100 minuti}   \\
		 & X_4 = 7000 \quad \quad \text{il 4º dura 7000 minuti}
	\end{align*}
	e sapendo che $X \sim Bernoulli(\theta)$ con $\Theta =  [0,1]$ allora la probabilità di osservare il campione $\vec{x}=(500,3000,100,7000)$ (in minuti) sarà:
	\begin{enumerate}
		\item Prendo 4 componenti nuovi
		\item Registro la loro durata e risulta che:
		      \begin{align*}
			      \begin{cases}
				      X_1 = 500  \\
				      X_2 = 2000 \\
			      ...
			      \end{cases}
		      \end{align*}
		      posso vedere che per $X_1,X_3$ risulta un successo (1), mentre per gli altri un insuccesso (0).
		      \begin{align*}
			      \begin{cases}
				      X_1 = 500  \\
				      X_2 = 2000 \\
		      ...
		      Quindi la probabilità di osservare
		      il campione $\vec{x}=(500,3000,100,7000)$ (in minuti) sarà:
		      \begin{align*}
			       & p_{\vec{X}}(\vec{x}; \theta)=                                                                                                                                                      \\
			       & =P(\vec{X}=\vec{x}; \theta)                                                                                                                                                        \\
			       & = P(X_1=1,X_2=0,X_3=1,X_4=0; \theta)                                                                                                                                               \\
			       & =\prod_{i=1}^{4}P(\vec{X_i}=\vec{x_i}; \theta)                                                                                                                                     \\
			       & =\prod_{i=1}^{4}\left(\theta^{x_i}(1-\theta)^{1-x_i}\right)                                                                                                                        \\
			       & =\left(\theta^{1}(1-\theta)^{1-1}\right) \cdot \left(\theta^{0}(1-\theta)^{1-0}\right) \cdot \left(\theta^{1}(1-\theta)^{1-1}\right) \cdot \left(\theta^{0}(1-\theta)^{1-0}\right) \\
			       & = \theta^{1+0+1+0}(1-\theta)^{(1+1+1+1)-(1+0+1+0)}                                                                                                                                 \\
			       & = \theta^{2}(1-\theta)^{4-(2)}                                                                                                                                                     \\
			       & = \theta^{2}(1-\theta)^2
		      \end{align*}
		      Ho trovato la funzione di verosomiglianza $\mathbf{L}(\theta) = \theta^{2}(1-\theta)^2$
		\item Applico la funzione di Log-verosomiglianza a $\mathbf{L}(\theta)$ ovvero:
		      \begin{align*}
			      \ell(\theta) = \ell(\mathbf{L}(\theta)) & = \ln\left[\theta^{2}(1-\theta)^2\right] \\
			                                              & = \ln(\theta^{2})+\ln((1-\theta)^2)      \\
			                                              & = 2\ln(\theta)+2\ln(1-\theta)
		      \end{align*}
		\item Calcolo derivata:
		      \begin{align*}
			      \ell'(\theta) & = (2\ln(\theta)+2\ln(1-\theta))'                                      \\
			                    & = (2\ln(\theta))'+(2\ln(1-\theta))'                                   \\
			                    & = (2 \cdot \frac{1}{\theta}) + (2 \cdot \frac{1}{1-\theta}\cdot (-1)) \\
			                    & = \frac{2}{\theta} - \frac{2}{1- \theta}
		      \end{align*}
		\item Impongo derivata uguale a zero:
		      \begin{align*}
			      \frac{2}{\theta} - \frac{2}{1- \theta} & = 0                                                     \\
			      \frac{1}{\theta} - \frac{1}{1- \theta} & = 0                                                     \\
			      \theta                                 & = 1-\theta                                              \\
			      \theta                                 & = 0.5 \quad \textcolor{green}{\text{Risposta corretta}}
		      \end{align*}
	\end{enumerate}
\end{esempio}

\subsection{Distribuzione $\chi^2$}
Utilizzo questa distribuzione quando ho un campione estratto da una popolazione Normale (ovvero quando i dati della popolazione si distribuiscono seguendo
la distribuzione Normale $X \sim N(\mu, \sigma^2)$).
\begin{theorem}{$X \sim \chi^2_\nu$}
	UUna variabile aleatoria continua $X$ ha distribuzione $\chi^2$ con $\nu$ gradi di libertà se non assume mai valori negativi:
	\begin{align*}
		\mathscr{X} = [0, +\infty)
	\end{align*}
\end{theorem}
\begin{theorem}{Media e Varianza di $X \sim \chi^2_\nu$}
	S\[
		\textcolor{red}{\mathbb{E}[X]}=\nu
	\]
	e
	\[
		\textcolor{red}{\mathbb{V}[X]}=2 \cdot \nu
	\]
\end{theorem}
\begin{center}
	\includegraphics[width=0.4\linewidth]{chi-quadro.png}
\end{center}
\textbf{Proprietà di $\chi^2$:}
\begin{enumerate}
	\item Dalla Normale a Chi-Quadro: ho una variabile normale standard $Z$ (quindi già standardizzata), se la elevo al quadrato ottengo una Chi-Quadro con 1 grado di libertà
	      \begin{align*}
		      Z^2 \sim \chi^2_{\nu=1}
	      \end{align*}
	      \begin{esempio}
		      Se ho $Z=\frac{x-\mu}{\sigma}=-1.5$, elevandolo al quadrato ottengo $P(Z^2 \leq 2.25)$ che equivale a $P(\sqrt{Z^2} \leq \sqrt{2.25})=P(|Z| \leq 1.5)=
			      P(-1.5 \leq Z \leq 1.5)$.
		      \begin{enumerate}
			      \item Se cerco nella tavola della Normale Standard:
			            \begin{align*}
				            P(-1.5 \leq Z \leq 1.5) & = \phi(1.5) - \phi(-1.5)        \\
				                                    & = \phi(1.5) - [1- \phi(1.5)]    \\
				                                    & = 0.9332 - [1- 0.9332] = 0.8664
			            \end{align*}
			      \item Se cerco nella tavola della Chi-Quadro:
			            \begin{align*}
				            P(\chi^2_\nu=1 \leq x_p) & = P(\chi^2_\nu=1 \leq 2.25)                                       \\
				                                     & \text{cerco il valore che sta in mezzo tra } 1.32 \text{ e } 2.71 \\
				                                     & \text{che corrispondono a } p=0.750 \text{ e } p=0.900            \\
				                                     & = \frac{0.750+0.900}{2}=0.825
			            \end{align*}
			            \begin{center}
				            \includegraphics[width=0.7\linewidth]{es-chi.png}
			            \end{center}
		      \end{enumerate}
	      \end{esempio}
	\item Proprietà della somma:
	      Se sommo diverse variabili Chi-Quadro indipendenti, ottengo ancora una Chi-Quadro:
	      \begin{align*}
		      \sum_{i=1}^{n} X_i \sim \chi^2_v \quad \text{ con } \quad \nu = \sum_{i=1}^{n}\nu_i
	      \end{align*}
	      \begin{esempio}
		      Siano:
		      \begin{align*}
		      ...
		      la loro somma sarà:
		      \begin{align*}
			      X_1+X_2+X_3 \text{ con } \nu = 2+3+5=10 \text{ gradi di libertà } \implies X_1+X_2+X_3 \sim \chi^2_{10}
		      \end{align*}
	      \end{esempio}
	\item Somma di normali standardizzate al quadrato: se ho $n$ variabili Normali (anche con medie $\mu_i$ e varianze $\sigma_i^2$ diverse), le standardizzo singolarmente, le elevo al quadrato,
	      poi le sommo. Il risultato equivale ad una $\chi^2$ con $\nu = n$ gradi di libertà
	      \begin{esempio}
		      Ho 3 sensori che misurano la temperatura (ognuno con media e varianza), e standardizzandoli ottengo:
		      \begin{align*}
			      \begin{cases}
				      Z_1 = 1 \implies 1^2=1       \\
				      Z_2 = -2 \implies (-2)^2 = 4 \\
				      Z_3 = 0.5 \implies 0.5^2 = 0.25
			      \end{cases}
		      \end{align*}
		      Sommandoli $1+4+0.25=5.25$ e quindi con Chi-Quadro avrò $x_p=5.25$ e $\nu=3$:
		      \begin{align*}
			      P(\chi^2_3 \leq 5.25) = 0.825
		      \end{align*}
	      \end{esempio}
\end{enumerate}

\begin{theorem}{Formule}
	SSia $X_1, \cdots, X_n$ un campione casuale da una popolazione Normale $X \sim N(\mu, \sigma^2)$ per calcolare la probabilità sulla somma, media e/o varianza uso le seguenti formule:
	\begin{enumerate}
		\item \textcolor{red}{Somma}
		      \begin{align*}
			      \sum_{i=1}^{n} X_i \sim N(n \cdot \mu, n \cdot \sigma^2)
		      \end{align*}
		      Uso tavole di Z
		\item \textcolor{red}{Media}
		      \begin{align*}
			      \overline{X} \sim N\left(\mu, \frac{\sigma^2}{n}\right)
		      \end{align*}
		      Uso tavole di Z
		\item \textcolor{red}{Varianza}
		      \begin{align*}
			      \frac{(n-1)\cdot S^2}{\sigma^2} \sim \chi^2_{\nu=n-1}
		      \end{align*}
		      Uso tavole di Chi-Quadro
	\end{enumerate}

\end{theorem}

\begin{esercizio}[Esercizio]
	Il diametro delle viti prodotte da un'azienda ha distribuzione Normale con $\mu = 150$ mm e varianza $\sigma^2=100$ mm$^2$. Siano $n=9$
	estratte dalla produzione (campione). Qual è la probabilità che la media campionaria assuma valori tra 145 e 155 mm?
	\begin{align*}
		\textcolor{red}{P(145 \leq \overline{X} \leq 155)} & = \phi\left(\frac{x_2-\mu}{\sqrt{\frac{\sigma^2}{n}}}\right) - \phi\left(\frac{x_1-\mu}{\sqrt{\frac{\sigma^2}{n}}}\right) \\
		                                                   & \text{devo calcolare la prob. sulla media uso proprietà 2 delle formule;}                                                 \\
		                                                   & \text{"standardizzo" usando la formula 2}                                                                                 \\
		                                                   & =  \phi\left(\frac{155-150}{\sqrt{\frac{100}{9}}}\right) - \phi\left(\frac{145-150}{\sqrt{\frac{100}{9}}}\right)          \\
		                                                   & =  \phi\left(\frac{155-150}{10/3}\right) - \phi\left(\frac{145-150}{10/3}\right)                                          \\
		                                                   & =  \phi(1.5) - \phi(-1.5)                                                                                                 \\
		                                                   & =  \phi(1.5) - [1-\phi(1.5)]                                                                                              \\
		                                                   & =  0.9332 - [1-0.9332] = 0.8664 \quad \textcolor{green}{\text{Risposta corretta}}
	\end{align*}
	Qual è la probabilità che la varianza campionaria sia maggiore di 121 mm?
	\begin{align*}
		\textcolor{red}{P(S^2>121)} & = P\left(\chi^2_{\nu=n-1} > \frac{(n-1)\cdot S^2}{\sigma^2}\right)           \\
		                            & \text{devo calcolare la prob. sulla varianza uso proprietà 3 delle formule;} \\
		                            & \text{"standardizzo" usando la formula 3}                                    \\
		                            & = P\left(\chi^2_{\nu=9-1} > \frac{(9-1)\cdot 121}{100}\right)                \\
		                            & = P\left(\chi^2_{\nu=8} > \frac{(8)\cdot 121}{100}\right)                    \\
		                            & = P\left(\chi^2_{8} > 9.68\right)                                            \\
		                            & = 1-  P\left(\chi^2_{8} \leq 9.68\right)                                     \\
		                            & = 1 - 0.700 = 0.3
	\end{align*}
	o meglio scrivere $1-  P\left(\chi^2_{8} \leq 9.68\right) \in (0.25, 0.5)$ \textcolor{green}{\text{Risposta corretta}}
\end{esercizio}
\subsection{Distribuzione $T-$Student}
Utilizzo questa distribuzione quando ho un campione estratto da una popolazione Normale (ovvero quando i dati della popolazione si distribuiscono seguendo
la distribuzione Normale $X \sim N(\mu, \sigma^2)$).
\begin{osservazioni}[Nota]
	Uso la distribuzione T-Student quando voglio calcolare la probabilità della media e ho $\sigma^2$ ignoto.
\end{osservazioni}
\begin{theorem}{Media e Varianza di $X \sim t_\nu$}
	S\[
		\textcolor{red}{\mathbb{E}[X]}=0
	\]
	e
	\[
		\textcolor{red}{\mathbb{V}[X]}=\frac{\nu}{\nu-2} \quad \text{ con } \nu>2
	\]
\end{theorem}
\begin{center}
	\includegraphics[width=0.4\linewidth]{t-student.png}
\end{center}
\begin{theorem}{Formule}
	SSia $X_1, \cdots, X_n$ un campione casuale da una popolazione Normale $X \sim N(\mu, \sigma^2)$ per calcolare la probabilità sulla media uso la seguente formula:
	\begin{align*}
		\frac{\sqrt{n}(\overline{X}-\mu)}{\sqrt{S^2}} \sim t_{\nu=n-1}
	\end{align*}
\end{theorem}
\begin{esercizio}[Esercizio]
	Il diametro delle viti prodotte da un'azienda ha distribuzione Normale con $\mu = 150$ mm e varianza $\sigma^2=100$ mm$^2$. Siano $n=9$
	estratte dalla produzione (campione). Qual è la probabilità che la media campionaria assuma valori che distano dalla media non più di $\left(1.86 \cdot \sqrt{\frac{\sigma^2}{n}}\right)$?
	\begin{itemize}
		\item Uso la Normale Standard perchè conosco $\sigma^2$:
		      \begin{align*}
			       & \textcolor{red}{P\left(\mu - \left(1.86 \cdot \sqrt{\frac{\sigma^2}{n}}\right) \leq \overline{X} \leq \mu + \left(1.86 \cdot \sqrt{\frac{\sigma^2}{n}}\right)\right)} = \\
			       & = P\left(\mu -1.86 \leq \overline{X} \cdot \sqrt{\frac{n}{\sigma^2}} \leq \mu + 1.86 \right)                                                                            \\
			       & = P\left(-1.86 \leq \overline{X} \cdot \sqrt{\frac{n}{\sigma^2}} - \mu \leq 1.86 \right)                                                                                \\
			       & = P\left(-1.86 \leq \frac{\overline{X}  - \mu}{\sqrt{\frac{\sigma^2}{n}}} \leq 1.86 \right)                                                                             \\
			       & = P\left(-1.86 \leq Z \leq 1.86 \right)                                                                                                                                 \\
			       & = \phi(z_2) - \phi(z_1)                                                                                                                                                 \\
			       & = \phi(1.86) - \phi(-1.86)                                                                                                                                              \\
			       & = \phi(1.86) - [1-\phi(-1.86)] = 0.9371
		      \end{align*}
		\item Uso la T-Student perchè $\sigma^2$ ignoto (suppongo che l'es non mi abbia dato il valore di $\sigma^2$):
		      \begin{align*}
			       & \textcolor{red}{P\left(\mu - \left(1.86 \cdot \sqrt{\frac{\sigma^2}{n}}\right) \leq \overline{X} \leq \mu + \left(1.86 \cdot \sqrt{\frac{\sigma^2}{n}}\right)\right)} = \\
			       & \sigma^2 \text{ è ignoto, quindi lo sostituisco con } S^2                                                                                                               \\
			       & P\left(\mu - \left(1.86 \cdot \sqrt{\frac{S^2}{n}}\right) \leq \overline{X} \leq \mu + \left(1.86 \cdot \sqrt{\frac{S^2}{n}}\right)\right) =                            \\
			       & \text{uso formula per standardizzare}                                                                                                                                   \\
			       & = P\left(-1.86 \leq \frac{\sqrt{n}(\overline{X}  - \mu)}{\sqrt{S^2}} \leq 1.86 \right)                                                                                  \\
			       & = P\left(-1.86 \leq T_{\nu=n-1} \leq 1.86 \right)                                                                                                                       \\
			       & = P\left(-1.86 \leq T_{\nu=8} \leq 1.86 \right)                                                                                                                         \\
			       & = P\left(-1.86 \leq T_{8} \leq 1.86 \right)                                                                                                                             \\
			       & = \phi(t_{p_2}) - \phi(t_{p_1})                                                                                                                                         \\
			       & = \phi(1.86) - \phi(-1.86)                                                                                                                                              \\
			       & = \phi(1.86) - [1-\phi(-1.86)]                                                                                                                                          \\
			       & = 0.95 - [1-0.95] = 0.90                                                                                                                                                \\
		      \end{align*}
		      \begin{center}
			      \includegraphics[width=0.6\linewidth]{es-t-stud.png}
		      \end{center}
	\end{itemize}
\end{esercizio}
\newpage
\begin{table}[h]
	...
	\begin{tabular}{|l|c|c|c|}
		\hline
		\textbf{Situazione}                                          & \textbf{Formula} & \textbf{Distribuzione} & \textbf{Tavola} \\[6pt]
		\hline
		Probabilità sulla \textbf{somma}, $\sigma^2$ nota            &
		$\dfrac{\displaystyle\sum X_i - n\mu}{\sqrt{n\sigma^2}}$     &
		$N(0,1)$                                                     &
		Normale standard                                                                                                           \\[6pt]
		\hline
		Probabilità sulla \textbf{media}, $\sigma^2$ nota            &
		$\dfrac{\overline{X} - \mu}{\sqrt{\sigma^2/n}}$              &
		$N(0,1)$                                                     &
		Normale standard                                                                                                           \\[6pt]
		\hline
		Probabilità sulla \textbf{varianza}                          &
		$\dfrac{(n-1)S^2}{\sigma^2}$                                 &
		$\chi^2_{n-1}$                                               &
		Chi-quadro                                                                                                                 \\[6pt]
		\hline
		Probabilità sulla \textbf{media}, $\sigma^2$ \textbf{ignota} &
		$\dfrac{\sqrt{n}(\overline{X} - \mu)}{\sqrt{S^2}}$           &
		$t_{n-1}$                                                    &
		T-Student                                                                                                                  \\[6pt]
		\hline
	\end{tabular}
	\caption{Quale distribuzione usare}
\end{table}
\newpage
\subsection{Teorema del Limite Centrale}
Questo teorema semplifica le cose quando voglio calcolare la probabilità della somma e della media di variabili aleatorie quando ho un campione di grandi dimensioni. Ho la possibilità
di utilizzare la Distribuzione Normale per approssimare il comportamento di somme e media campionarie, quando non conosco la distribuzione della popolazione di partenza (oppure so con
certezza che non ha distribuzione normale).
Utile quando:
\begin{itemize}
	\item La popolazione di partenza non ha Distribuzione Normale oppure non so che distribuzione abbia (può essere di Bernoulli, Binomiale, ...)
	\item Il campione è di grandi dimensioni
	\item La varianza $\sigma^2$ è finita e maggiore di zero
\end{itemize}
\begin{theorem}{Teorema del Limite Centrale}
	SSiano una successione di variabili aleatorie i.i.d con valore atteso $\mathbb{E}[X_i]=\mu$ e varianza $0<\mathbb{V}[X_i]=\sigma^2 < + \infty$, allora:
	\begin{enumerate}
		\item La variabile standardizzata della media campionaria converge ad una Normale Standard:
		      \begin{align*}
			      \frac{\sqrt{n}(\overline{X}_n - \mu)}{\sqrt{\sigma^2}} \textcolor{cyan}{\overset{d}{\longrightarrow}} N(0,1)
		      \end{align*}
		\item La variabile standardizzata della somma converge ad una Normale Standard:
		      \begin{align*}
			      \frac{\sum^{n}_{i=1} \left(X_i\right) - n \mu}{\sqrt{n\sigma^2}} \textcolor{cyan}{\overset{d}{\longrightarrow}} N(0,1)
		      \end{align*}
		\item La variabile standardizzata della media campionaria con $\sigma^2$ ignoto, converge ad una Normale Standard:
		      \begin{align*}
			      \frac{\sqrt{n}(\overline{X}_n - \mu)}{\sqrt{S^2}} \textcolor{cyan}{\overset{d}{\longrightarrow}} N(0,1)
		      \end{align*}
	\end{enumerate}
\end{theorem}
\begin{osservazioni}[Come usare il Teorema del Limite Centrale?]
	Se voglio calcolare la probabilità di una somma o media di una popolazione con distribuzione non Normale (Bernoulli, Binomiale, Poisson, ...) con campione sufficientemente grande,
	posso standardizzare le variabili aleatorie
	e poi calcolare con la tavola di Z. Ricordo che:
	\begin{align*}
		\underset{\text{Media}}{\frac{\sqrt{n}(\overline{X}_n - \mu)}{\sqrt{\sigma^2}} \, \textcolor{cyan}{\simeq} \, N(0,1)}
		\quad \quad \quad \quad
		\underset{\text{Somma}}{\frac{\sum^{n}_{i=1} \left(X_i\right) - n \mu}{\sqrt{n\sigma^2}} \, \textcolor{cyan}{\simeq} \, N(0,1)}
	\end{align*}
	\begin{align*}
		\underset{\text{Media}}{\overline{X}_n \simeq N\left(\mu, \frac{\sigma^2}{n}\right)}
		\quad \quad \quad \quad
		\underset{\text{Somma}}{\sum X_i \simeq N\left(n\mu, n\sigma^2\right)}
	\end{align*}
	e che:
	\begin{align*}
		Z = \frac{x-\mu}{\sqrt{\sigma^2}} \quad \quad \quad N(\mu, \sigma^2)
	\end{align*}
	quindi per standardizzare i valori faccio:
	\begin{align*}
		\underset{\text{Media}}{\phi\left(\frac{\sqrt{n}(x - \mu)}{\sqrt{\sigma^2}}\right)}
		\quad \quad \quad \quad
		\underset{\text{Somma}}{\phi\left(\frac{x - n \mu}{\sqrt{n\sigma^2}}\right)}
	\end{align*}
	e poi guardo le tavole di Z e trovo i valori.
\end{osservazioni}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Stima Puntuale}
Siano i dati osservati
\begin{align*}
	\vec{x} = (x_1, \cdots, x_n)
\end{align*}
la realizzazione di un campione casuale
\begin{align*}
	\vec{X} = \left(X_1, \cdots, X_n \right)
\end{align*}
da una popolazione $X$ (Normale, Binomiale, Bernoulliana, Poisson, ecc...). \\
La \textcolor{red}{stima puntuale} risponde alla domanda: "qual è il valore del parametro ($\mu, \theta, \cdots$) che ha generato i dati che sto osservando?"
\subsubsection{Funzione di Verosomiglianza}
\begin{theorem}{funzione di verosomiglianza}
	SSia $\vec{X}=(X_1, \cdots, X_n)$ un campione con funzione di massa/densità di probabilità congiunta
	\begin{align*}
		 & p_{\vec{X}}(\cdot;\theta) = p_{(X_1,\cdots,X_n)}(\cdot;\theta)=P(X_1=x_1, \cdots, X_n=x_n) \\
		 & f_{\vec{X}}(\cdot;\theta) = f_{(X_1,\cdots,X_n)}(\cdot;\theta)
	\end{align*}
	e sia $\vec{x}=(x_1,\cdots, x_n)$ una sua realizzazione. \\
	Si chiama \textcolor{red}{funzione di verosomiglianza} associata al campione osservato $\vec{x}$ la funzione:
	\begin{align*}
		\mathscr{L}(\theta) = \mathscr{L}(\theta; \vec{x}) = \begin{cases}
			                                                     \prod_{i=1}^{n}p_X(x_i;\theta) \quad \text{se } X \text{ discreta} \\
			                                                     \prod_{i=1}^{n}f_X(x_i;\theta) \quad \text{se } X \text{ continua}
		                                                     \end{cases}
	\end{align*}
\end{theorem}
La funzione di verosomiglianza è il prodotto delle $n$ funzioni di massa/densità di probabilità.

\subsubsection{Funzione di Log-Verosomiglianza}
\begin{theorem}{funzione di log-verosomiglianza}
	SSi chiama \textcolor{red}{funzione di log-verosomiglianza} associata al campione osservato $\vec{x}$ la funzione logaritmo naturale che prende come input la funzione
	di verosomiglianza:
	\begin{align*}
		\ell(\theta) = \ell(\mathscr{L}(\theta; \vec{x})) =\ln(\mathscr{L}(\theta; \vec{x})) = \begin{cases}
			                                                                                       \sum_{i=1}^{n}\left(\ln\left(p_X(x_i;\theta)\right)\right) \quad \text{se } X \text{ discreta} \\
			                                                                                       \sum_{i=1}^{n}\left(\ln\left(f_X(x_i;\theta)\right)\right) \quad \text{se } X \text{ continua}
		                                                                                       \end{cases}
	\end{align*}
\end{theorem}
La funzione di log-verosomiglianza è la somma delle $n$ funzioni di massa/densità di probabilità a cui viene applicato il logaritmo naturale (prima applico $\ln$ a ogni funzione di massa/
densità di probabilità e poi le sommo).
\subsubsection{Metodo della Massima Verosomiglianza}
Serve per trovare il valore del parametro ignoto.
\begin{enumerate}
	\item \textcolor{red}{Trovo la funzione di verosomiglianza:}
	      \begin{align*}
		      \mathscr{L}(\theta; \vec{x}) = \begin{cases}
			                                     \prod_{i=1}^{n}p_X(x_i;\theta) \quad \text{se } X \text{ discreta} \\
			                                     \prod_{i=1}^{n}f_X(x_i;\theta) \quad \text{se } X \text{ continua}
		                                     \end{cases}
	      \end{align*}
	\item \textcolor{red}{Trovo la funzione di log-verosomiglianza applicando il logaritmo naturale alla funzione del punto 1:}
	      \begin{align*}
		      \ell(\theta) = \ell(\mathscr{L}(\theta; \vec{x})) =\ln(\mathscr{L}(\theta; \vec{x}))
	      \end{align*}
	\item \textcolor{red}{Calcolo la derivata di $\ell(\theta)$ del punto 2}
	\item \textcolor{red}{Impongo la derivata uguale a zero (trovo il punto stazionario):}
	      \begin{align*}
		      \ell'(\theta)=0
	      \end{align*}
	\item \textcolor{red}{Verifico che la derivata seconda sia negativa:}
	      il punto stazionario è un punto di massimo se la derivata seconda è negativa
	      \begin{itemize}
		      \item Se il parametro ignoto è solo uno, controllo che la derivata seconda sia negativa (il punto stazionario è un punto di massimo)
		            \begin{align*}
			            \ell''(\theta)<0
		            \end{align*}
		      \item Se i parametri ignoti sono due, controllo che la derivata parziale seconda sia negativa con il test dell'Hessiana
	      \end{itemize}
\end{enumerate}
\begin{esempio}[Esempio: Stima di MV per il modello di Poisson]
	Sia $X_1,\cdots, X_n$ un campione casuale da una popolazione $X$ con $X \sim$ Possion $(\lambda)$.
	\begin{enumerate}
		\item Trovo la funzione di verosomiglianza
		      \begin{align*}
			      \mathscr{L}(\lambda; \vec{x}) & = \prod_{i=1}^{n}\left(e^{-\lambda}\frac{\lambda^{x_i}}{x_i!}\right)                                                                                                      \\
			                                    & = \left(e^{-\lambda}\frac{\lambda^{x_1}}{x_1!}\right) \cdot \left(e^{-\lambda}\frac{\lambda^{x_2}}{x_2!}\right) \dots \left(e^{-\lambda}\frac{\lambda^{x_n}}{x_n!}\right) \\
			                                    & = e^{-n\lambda}\frac{\lambda^{(x_1+x_2+\dots+x_n)}}{x_1! \cdot x_2! \cdots x_n!}                                                                                          \\
			                                    & = e^{-n\lambda}\frac{\lambda^{\left(\sum x_i\right)}}{\prod x_i!}
		      \end{align*}
		\item Trovo la funzione di log-verosomiglianza
		      \begin{align*}
			      \ell(\lambda; \vec{x}) & = \ln\left(e^{-n\lambda} \cdot \frac{\lambda^{\left(\sum x_i\right)}}{\prod x_i!}\right)                       \\
			                             & \text{per le proprietà dei logaritmi}                                                                          \\
			                             & = \ln\left(e^{-n\lambda}\right) + \ln\left(\frac{\lambda^{\left(\sum x_i\right)}}{\prod x_i!}\right)           \\
			                             & = \ln\left(e^{-n\lambda}\right) + \ln\left(\lambda^{\left(\sum x_i\right)}\right) - \ln\left(\prod x_i!\right) \\
			                             & = -n \lambda \cdot \ln(e) + \sum x_i \cdot \ln(\lambda) - \ln \left(\prod x_i! \right)                         \\
			                             & \text{so che } \ln(e)=1 \text{ quindi}                                                                         \\
			                             & = -n \lambda + \sum x_i \cdot \ln(\lambda) - \ln \left(\prod x_i! \right)
		      \end{align*}
		\item Calcolo la derivata \\
		      in questo caso devo derivare rispetto a $\lambda$ quindi tutto ciò che non contiene $\lambda$ scompare:
		      \begin{align*}
			      \ell'(\lambda) & = \left(-n \lambda + \sum x_i \cdot \ln(\lambda) - \ln \left(\prod x_i! \right)\right)'                   \\
			                     & = \left(-n \lambda\right)' + \left(\sum x_i \cdot \ln(\lambda)\right)'                                    \\
			                     & = -n + \left[\left(\sum x_i \cdot \ln(\lambda)\right) + \left(\sum x_i \cdot \ln(\lambda)\right)\right]'  \\
			                     & = -n + \left[\left(\sum x_i' \cdot \ln(\lambda)\right) + \left(\sum x_i \cdot \ln(\lambda)'\right)\right] \\
			                     & = -n + \left(\sum x_i \cdot \ln(\lambda)'\right)                                                          \\
			                     & = -n + \left(\sum x_i \cdot \frac{1}{\lambda}\right)                                                      \\
			                     & = -n + \left(\frac{\sum x_i}{\lambda}\right)
		      \end{align*}
		\item Impongo derivata prima uguale a zero e trovo stimatore
		      \begin{align*}
			      -n + \left(\frac{\sum x_i}{\lambda}\right) & = 0                  \\
		      ...
		      La stima è
		      \begin{align*}
			      \textcolor{red}{\hat{\lambda}} = \frac{\sum x_i}{n}
		      \end{align*}
		      Lo stimatore è
		      \begin{align*}
			      \textcolor{red}{\hat{\lambda}} = \frac{\sum X_i}{n}
		      \end{align*}
		\item Verifico che la derivata seconda sia negativa \\
		      Il parametro ignoto è solo uno, quindi basta calcolare la derivata seconda
		      \begin{align*}
			      (\ell'(\lambda))' & = \left[ -n + \left(\frac{\sum x_i}{\lambda}\right) \right]'                                                  \\
			                        & = (-n)' + \left(\frac{\sum x_i}{\lambda}\right)'                                                              \\
			                        & = \left(\frac{\sum x_i}{\lambda}\right)'                                                                      \\
			                        & = \left[\frac{\left(\sum x_i' \cdot \lambda\right) - \left(\sum x_i \cdot \lambda'\right)}{\lambda^2} \right] \\
			                        & = - \frac{\left(\sum x_i \cdot \lambda'\right)}{\lambda^2}                                                    \\
			                        & = - \frac{\sum x_i}{\lambda^2}
		      \end{align*}
		      È negativa $(\ell'(\lambda))' < 0$ quindi la stima di MV di $\lambda$ è $\textcolor{red}{\hat{\lambda}} = \frac{\sum x_i}{n}$ (che alla fine è la media), quindi:
		      \begin{align*}
			      \text{Stima di MV di $\lambda$} \quad \quad \hat{\lambda}_{ML}(\vec{x}) = \frac{\sum x_i}{n}=\overline{x}
		      \end{align*}
		      \begin{align*}
			      \text{Stimatore di MV di $\lambda$} \quad \quad \hat{\lambda}_{ML}(\vec{X}) = \frac{\sum X_i}{n}=\overline{X}
		      \end{align*}
	\end{enumerate}
\end{esempio}
\begin{esempio}[Esempio: Stima di MV per il modello Normale]
	La differenza tra questo esempio e quello precedente è che il parametro ignoto della Normale è un vettore $\theta=(\mu, \sigma^2)$ quindi invece di trovare la derivata prima, devo
	trovare le derivate parziali (punto 3) , imporre le derivate parziali uguali a zero in un sistema (punto 4) e successivamente verificare che la stima trovata sia quella giusta controllando
	che la derivata parziale seconda sia negativa con il test dell'Hessiana. \\ \\
	Siano $X_1, \dots, X_n$ variabili aleatorie i.i.d con $X \sim N(\mu, \sigma^2)$.
	\begin{enumerate}
		\item Trovo la funzione di verosomiglianza
		      \begin{align*}
			      \mathscr{L}(\mu, \sigma^2; \vec{x}) & = \prod_{i=1}^{n}\frac{1}{\sqrt{2 \pi \sigma^2}}\cdot \mathrm{e}^{-\frac{1}{2}\left(\frac{x_i-\mu}{\sigma}\right)^2}                                                                          \\
			                                          & = \prod_{i=1}^{n}\frac{1}{\sqrt{2 \pi}} \cdot \frac{1}{\sqrt{\sigma^2}} \cdot \mathrm{e}^{-\frac{1}{2}\left(\frac{x_i-\mu}{\sigma}\right)^2}                                                  \\
			                                          & = \left(\frac{1}{\sqrt{2 \pi}}\right)^n \cdot \left(\frac{1}{\sqrt{\sigma^2}}\right)^n \cdot \mathrm{e}^{\sum \left(-\frac{1}{2}\left(\frac{x_i-\mu}{\sigma}\right)^2\right)}                 \\
			                                          & = \left(\frac{1}{\sqrt{2 \pi}}\right)^n \cdot \left(\frac{1}{\sqrt{\sigma^2}}\right)^n \cdot \mathrm{e}^{-\frac{1}{2\sigma^2} \sum \left(\left({x_i-\mu}\right)^2\right)}                     \\
			                                          & = \left(\frac{1}{\sqrt{2 \pi}}\right)^n \cdot \left(\frac{1}{\left(\sigma^2\right)^\frac{1}{2}}\right)^n \cdot \mathrm{e}^{-\frac{1}{2\sigma^2} \sum \left(\left({x_i-\mu}\right)^2\right)}   \\
			                                          & = \left(\frac{1}{\sqrt{2 \pi}}\right)^n \cdot \frac{1^n}{\left(\left(\sigma^2\right)^\frac{1}{2}\right)^n} \cdot \mathrm{e}^{-\frac{1}{2\sigma^2} \sum \left(\left({x_i-\mu}\right)^2\right)} \\
			                                          & = \left(\frac{1}{\sqrt{2 \pi}}\right)^n \cdot \frac{1^n}{\left(\sigma^2\right)^\frac{n}{2}} \cdot \mathrm{e}^{-\frac{1}{2\sigma^2} \sum \left(\left({x_i-\mu}\right)^2\right)}                \\
			                                          & = \left(\frac{1}{\sqrt{2 \pi}}\right)^n \cdot \left(\frac{1}{\sigma^2}\right)^\frac{n}{2} \cdot \mathrm{e}^{-\frac{1}{2\sigma^2} \sum \left(\left({x_i-\mu}\right)^2\right)}                  \\
			                                          & = \left(\frac{1}{\sqrt{2 \pi}}\right)^n \cdot \left(\sigma^2\right)^{-\frac{n}{2}} \cdot \mathrm{e}^{-\frac{1}{2\sigma^2} \sum \left(\left({x_i-\mu}\right)^2\right)}
		      \end{align*}
		\item Trovo la funzione di log-verosomiglianza
		      \begin{align*}
			      \ell(\mu, \sigma^2; \vec{x}) & = \ln \left(\left(\frac{1}{\sqrt{2 \pi}}\right)^n \cdot \left(\sigma^2\right)^{-\frac{n}{2}} \cdot \mathrm{e}^{-\frac{1}{2\sigma^2} \sum \left(\left({x_i-\mu}\right)^2\right)}\right)                        \\
			                                   & = \ln\left[\left(\frac{1}{\sqrt{2 \pi}}\right)^n\right] + \ln\left[\left(\sigma^2\right)^{-\frac{n}{2}}\right] + \ln\left[\mathrm{e}^{-\frac{1}{2\sigma^2} \sum \left(\left({x_i-\mu}\right)^2\right)}\right] \\
			                                   & = n \ln \left(\frac{1}{\sqrt{2 \pi}}\right) - \frac{n}{2} \ln \left(\sigma^2\right) -\frac{1}{2\sigma^2} \sum \left(\left({x_i-\mu}\right)^2\right) \ln (e)                                                   \\
			                                   & = n \ln \left(\frac{1}{\sqrt{2 \pi}}\right) - \frac{n}{2} \ln \left(\sigma^2\right) -\frac{1}{2\sigma^2} \sum \left(\left({x_i-\mu}\right)^2\right)                                                           \\
			                                   & = - n \ln \left(\sqrt{2 \pi}\right) - \frac{n}{2} \ln \left(\sigma^2\right) -\frac{1}{2\sigma^2} \sum \left(\left({x_i-\mu}\right)^2\right)                                                                   \\
			                                   & = - n \ln \left({2 \pi}^{1/2}\right) - \frac{n}{2} \ln \left(\sigma^2\right) -\frac{1}{2\sigma^2} \sum \left(\left({x_i-\mu}\right)^2\right)                                                                  \\
			                                   & = - \frac{1}{2} n \ln \left({2 \pi}\right) - \frac{n}{2} \ln \left(\sigma^2\right) -\frac{1}{2\sigma^2} \sum \left(\left({x_i-\mu}\right)^2\right)                                                            \\
			                                   & = - \frac{n}{2} \ln \left({2 \pi}\right) - \frac{n}{2} \ln \left(\sigma^2\right) -\frac{1}{2\sigma^2} \sum \left(\left({x_i-\mu}\right)^2\right)                                                              \\
		      \end{align*}
		\item Calcolo derivate prime parziali
		      \begin{itemize}
			      \item Derivata prima parziale rispetto a $\mu$ (quindi $\sigma^2, n$ costanti)
			            \begin{align*}
				            \frac{\partial \ell}{\partial \mu} & = \left(- \frac{n}{2} \ln \left({2 \pi}\right) - \frac{n}{2} \ln \left(\sigma^2\right) -\frac{1}{2\sigma^2} \sum \left(\left({x_i-\mu}\right)^2\right)\right)' \\
				                                               & = - 0 - 0 -  \left(\frac{1}{2\sigma^2} \sum \left(\left({x_i-\mu}\right)^2\right)\right)'                                                                      \\
				                                               & = -  \left(\frac{1}{2\sigma^2} \sum \left(-2\left({x_i-\mu}\right)\right)\right)                                                                               \\
				                                               & = \frac{1}{\sigma^2} \sum \left({x_i-\mu}\right)                                                                                                               \\
				                                               & \text{sapendo che } \mu = n\mu                                                                                                                                 \\
				                                               & = \frac{1}{\sigma^2} \sum \left({x_i-n\mu}\right)
			            \end{align*}
			      \item Derivata prima parziale rispetto a $\sigma^2$ (quindi $\mu, n$ costanti)
			            \begin{align*}
				            \frac{\partial \ell}{\partial \sigma^2} & = \left(- \frac{n}{2} \ln \left({2 \pi}\right) - \frac{n}{2} \ln \left(\sigma^2\right) -\frac{1}{2\sigma^2} \sum \left(\left({x_i-\mu}\right)^2\right)\right)' \\
				                                                    & = 0 - \left(\frac{n}{2} \ln \left(\sigma^2\right)\right)' -  \left(\frac{1}{2\sigma^2} \sum \left(\left({x_i-\mu}\right)^2\right)\right)'                      \\
				                                                    & \text{ per evitare di impazzire impongo } \sigma^2=Y                                                                                                           \\
				                                                    & = - \left(\frac{n}{2} \ln \left(Y\right)\right)' -  \left(\frac{1}{2Y} \sum \left(\left({x_i-\mu}\right)^2\right)\right)'                                      \\
				                                                    & = - \frac{n}{2Y} + \frac{1}{2Y^2} \sum \left({x_i-\mu}\right)^2                                                                                                \\
				                                                    & \text{sostituisco } Y                                                                                                                                          \\
				                                                    & = - \frac{n}{2\sigma^2} + \frac{1}{2(\sigma^2)^2} \sum \left({x_i-\mu}\right)^2                                                                                \\
				                                                    & = \frac{1}{2\sigma^2} \left[-n + \frac{1}{\sigma^2} \sum \left({x_i-\mu}\right)^2 \right]
			            \end{align*}
		      \end{itemize}
		\item Impongo derivate prime parziali uguali a zero col sistema e trovo stimatori $\hat{\mu}, \hat{\sigma}^2$
		      \begin{enumerate}
			      \item \textcolor{red}{trovo $\hat{\mu}$}
			            \begin{align*}
				            \dfrac{\partial \ell}{\partial \mu}             & = 0                  \\
				            \dfrac{1}{\sigma^2} \sum x_i - n\mu             & = 0                  \\
				            \frac{\sum x_i - n\mu}{\sigma^2}                & = 0                  \\
				            \sigma^2 \cdot \frac{\sum x_i - n\mu}{\sigma^2} & = \sigma^2 \cdot 0   \\
				            \sum x_i - n\mu                                 & = 0                  \\
				            \mu                                             & = \frac{\sum x_i}{n}
			            \end{align*}
			            Quindi la stima è la media:
			            \begin{align*}
				            \textcolor{red}{\hat{\mu}} = \frac{\sum x_i}{n} = \overline{x}
			            \end{align*}
			      \item \textcolor{red}{trovo $\hat{\sigma}^2$}
			            \begin{align*}
				            \dfrac{\partial \ell}{\partial \sigma^2}                                        & = 0                                                 \\
				            \dfrac{1}{2\sigma^2} \left[ -n + \dfrac{1}{\sigma^2} \sum (x_i - \mu)^2 \right] & = 0                                                 \\
				            \vdots                                                                                                                                \\
				            \sigma^2                                                                        & = \frac{1}{n} \sum \left(x_i- \mu\right)^2          \\
				            \text{sostiuisco sapendo che } \textcolor{cyan}{{\mu}} = \frac{\sum x_i}{n} = \overline{x} \text{ vedi passo prec.}                   \\
				            \sigma^2                                                                        & = \frac{1}{n} \sum \left(x_i- \overline{x}\right)^2 \\
			            \end{align*}
			            Quindi la stima è la varianza:
			            \begin{align*}
				            \textcolor{red}{\hat{\sigma}^2}= \frac{1}{n} \sum \left(x_i- \overline{x}\right)^2
			            \end{align*}
		      \end{enumerate}
		\item Verifico che la derivata seconda parziale sia negativa con il test dell'Hessiana
		      \begin{itemize}
			      \item \textbf{Calcolo delle derivate parziali seconde:}
			            \begin{align*}
				            \dfrac{\partial^2 \ell}{\partial \mu^2}                    & = \dfrac{\partial}{\partial \mu} \left[ \dfrac{1}{\sigma^2} \left( \sum_{i=1}^n x_i - n\mu \right) \right] = -\dfrac{n}{\sigma^2}                                                                              \\[10pt]
				            \dfrac{\partial^2 \ell}{\partial (\sigma^2)^2}             & = \dfrac{\partial}{\partial \sigma^2} \left[ -\dfrac{n}{2\sigma^2} + \dfrac{1}{2(\sigma^2)^2} \sum_{i=1}^n (x_i - \mu)^2 \right] = \dfrac{n}{2(\sigma^2)^2} - \dfrac{\sum_{i=1}^n (x_i - \mu)^2}{(\sigma^2)^3} \\[10pt]
				            \dfrac{\partial^2 \ell}{\partial \mu \, \partial \sigma^2} & = \dfrac{\partial}{\partial \sigma^2} \left[ \dfrac{1}{\sigma^2} \left( \sum_{i=1}^n x_i - n\mu \right) \right] = -\dfrac{1}{(\sigma^2)^2} \left( \sum_{i=1}^n x_i - n\mu \right)
			            \end{align*}

			      \item \textbf{Valutazione nel punto stazionario $(\hat{\mu}, \hat{\sigma}^2)$:}
			            Sostituendo le stime di massima verosimiglianza $\hat{\mu} = \overline{x}$ (da cui $\sum x_i = n\overline{x}$) e $\hat{\sigma}^2 = \frac{1}{n}\sum(x_i-\overline{x})^2$ (da cui $\sum(x_i-\overline{x})^2 = n\hat{\sigma}^2$):
			            \begin{align*}
				            \left. \dfrac{\partial^2 \ell}{\partial \mu^2} \right|_{(\hat{\mu}, \hat{\sigma}^2)}                    & = -\dfrac{n}{\hat{\sigma}^2}                                                                                                                                                                                                             \\[10pt]
				            \left. \dfrac{\partial^2 \ell}{\partial \mu \, \partial \sigma^2} \right|_{(\hat{\mu}, \hat{\sigma}^2)} & = -\dfrac{1}{(\hat{\sigma}^2)^2} \underbrace{\left( \sum_{i=1}^n x_i - n\overline{x} \right)}_{= \, 0} = 0                                                                                                                               \\[10pt]
				            \left. \dfrac{\partial^2 \ell}{\partial (\sigma^2)^2} \right|_{(\hat{\mu}, \hat{\sigma}^2)}             & = \dfrac{n}{2(\hat{\sigma}^2)^2} - \dfrac{\overbrace{\sum_{i=1}^n (x_i - \overline{x})^2}^{= \, n\hat{\sigma}^2}}{(\hat{\sigma}^2)^3} = \dfrac{n}{2(\hat{\sigma}^2)^2} - \dfrac{n}{(\hat{\sigma}^2)^2} = -\dfrac{n}{2(\hat{\sigma}^2)^2}
			            \end{align*}

			      \item \textbf{Costruzione della Matrice Hessiana $H(\hat{\mu}, \hat{\sigma}^2)$:}
			            Poiché le derivate miste si azzerano, la matrice assume una struttura diagonale:
			            \begin{align*}
				            H(\hat{\mu}, \hat{\sigma}^2) =
				            \begin{bmatrix}
					            \left. \dfrac{\partial^2 \ell}{\partial \mu^2} \right.                    & \left. \dfrac{\partial^2 \ell}{\partial \mu \, \partial \sigma^2} \right. \\[15pt]
					            \left. \dfrac{\partial^2 \ell}{\partial \sigma^2 \, \partial \mu} \right. & \left. \dfrac{\partial^2 \ell}{\partial (\sigma^2)^2} \right.
				            \end{bmatrix}
				            =
				            \begin{bmatrix}
					            -\dfrac{n}{\hat{\sigma}^2} & 0                               \\[15pt]
					            0                          & -\dfrac{n}{2(\hat{\sigma}^2)^2}
				            \end{bmatrix}
			            \end{align*}

			      \item \textbf{Test del Determinante:}
			            Il determinante dell'Hessiana è positivo e \\ $-\dfrac{n}{\hat{\sigma}^2} < 0$ quindi la stima di MV di $(\mu, \sigma^2)$ è $(\hat{\mu}, \hat{\sigma}^2)$.
			            Ovvero le stime trovate nel punto 4 sono giuste.
		      \end{itemize}
	\end{enumerate}
\end{esempio}
\subsubsection{Metodi per valutare gli stimatori}
Ho trovato lo stimatore per il parametro ignoto e voglio verificare le sue proprietà:
\begin{itemize}
	\item Non distorsione
	\item Efficienza
	\item Consistenza
\end{itemize}
\begin{theorem}{Non distorsione}
	SSia $T_n(\vec{X})$ uno stimatore per il parametro $\theta$ di un certo modello statistico. Lo stimatore è detto \textcolor{red}{non distorto} se:
	\begin{align*}
		\mathbb{E}_{\theta}[T_n(\vec{X})] = \theta
	\end{align*}
	E \textcolor{red}{distorto} se:
	\begin{align*}
		\mathbb{E}_{\theta}[T_n(\vec{X})] \neq \theta
	\end{align*}
	Nel caso in cui lo stimatore fosse distorto, si calcola il valore della \textcolor{red}{distorsione} come segue:
	\begin{align*}
		\mathbb{B}_{\theta}(T_n(\vec{X})) = \mathbb{E}_{\theta}[T_n(\vec{X})] - \theta
	\end{align*}
\end{theorem}
\begin{esempio}[Stimatore non distorto]
	Nell'esempio precedente con il metodo di MV ho trovato sia $\hat{\lambda}=\frac{\sum X_i}{n}$ è lo stimatore del parametro $\lambda$ con Distribuzione di Poisson. \\
	\textcolor{red}{Lo stimatore è distorto o non distorto?}
	\begin{align*}
		T_n(\vec{X}) = \hat{\lambda}=\frac{\sum X_i}{n} \implies T_n(\vec{X}) = \hat{\lambda}
	\end{align*}
	Verifico uguaglianza:
	\begin{align*}
		\mathbb{E}[T_n(\vec{X})] \overset{?}{=} \lambda                                        \\
		\mathbb{E}[\hat{\lambda}] \overset{?}{=} \lambda                                       \\
		\mathbb{E}\left[\frac{\sum X_i}{n}\right] \overset{?}{=} \lambda                       \\
		\frac{1}{n}\mathbb{E}\left[\sum X_i\right] \overset{?}{=} \lambda                      \\
		\frac{1}{n}\mathbb{E}\left[X_1+\dots+X_n\right] \overset{?}{=} \lambda                 \\
		\frac{1}{n}\left(\mathbb{E}[X_1]+\dots+ \mathbb{E} [X_n]\right) \overset{?}{=} \lambda \\
		\text{nella Poisson, } \mathbb{E}[X]=\lambda \text{, quindi}                           \\
		\frac{1}{n}\left(\lambda+\dots+ \lambda \right) \overset{?}{=} \lambda                 \\
		\frac{1}{n}\left(n \lambda \right) \overset{?}{=} \lambda                              \\
		\lambda \overset{\textcolor{green}{\checkmark}}{=} \lambda                             \\
	\end{align*}
	L'uguaglianza è vera, quindi lo stimatore è non distorto.
\end{esempio}
\begin{esempio}[Stimatore distorto]
	Suppongo che lo stimatore trovato sia $\hat{\lambda}=\frac{1}{n}\sum \left(X_i - \overline{X}\right)^2$ verifico se è distorto oppure no:
	\begin{align*}
		\mathbb{E}[T_n(\vec{X})] \overset{?}{=} \lambda                                                  \\
		\mathbb{E}\left[\frac{1}{n}\sum \left(X_i - \overline{X}\right)^2\right] \overset{?}{=} \lambda  \\
		\frac{1}{n} \mathbb{E}\left[\sum \left(X_i - \overline{X}\right)^2\right] \overset{?}{=} \lambda \\
		\frac{n-1}{n}\lambda \textcolor{red}{\neq} \lambda
	\end{align*}
	Lo stimatore è distorto, calcolo il valore della distorsione:
	\begin{align*}
		\mathbb{B}_{\lambda}(T_n(\vec{X})) & = \mathbb{E}[T_n(\vec{X})] - \lambda \\ &=  \frac{n-1}{n}\lambda - \lambda \\
		                                   & = - \frac{\lambda}{n}
	\end{align*}
	Nel caso avessi $n=5, \lambda=4$ allora la distorsione sarebbe:
	\begin{align*}
		\mathbb{B}_{\lambda}(T_n(\vec{X})) = - \frac{\lambda}{n} =- \frac{4}{5} = -0.8
	\end{align*}
\end{esempio}
\begin{theorem}{Efficienza}
	SSia $T$ uno stimatore per il parametro $\theta$ di un certo modello statistico. L'\textcolor{red}{errore quadratico medio} di T viene definito come:
	\begin{align*}
		MSE_{\theta}(T) = \mathbb{V}_{\theta}[T(\vec{X})] + \mathbb{B}_{\theta}(T(\vec{X}))^2
	\end{align*}
	e se il parametro non è distorto\footnote{perchè quando il parametro non è distorto, per definizione $\mathbb{B}_{\theta}(T(\vec{X}))=0$}:
	\begin{align*}
		MSE_{\theta}(T) = \mathbb{V}_{\theta}[T(\vec{X})]
	\end{align*}
\end{theorem}
Siano $T_1(\vec{X}), T_2(\vec{X})$ due stimatori del parametro $\theta$. Lo stimatore $T_1$ è \textcolor{red}{più efficiente} di $T_2$ se
\begin{align*}
	MSE_{\theta}(T_1(\vec{X})) \leq MSE_{\theta}(T_2(\vec{X}))
\end{align*}
e misuro \textcolor{red}{quanto è più efficiente} dalla formula:
\begin{align*}
	\text{eff}_{\theta}(T_1,T_2) = \frac{MSE_{\theta}(T_1(\vec{X}))}{MSE_{\theta}(T_2(\vec{X}))}
\end{align*}
Quindi nella prima indico qual è lo stimatore più efficiente tra i due e nella seconda di quanto (valore numerico) è più efficiente dell'altro.
\begin{theorem}{Consistenza}
	UUno stimatore $T(\vec{X})$ per il parametro $\theta$ di un certo modello statistico si dice \textcolor{red}{consistente} se:
	\begin{align*}
		\lim_{n \to \infty} MSE_{\theta}(T(\vec{X})) = 0
	\end{align*}
\end{theorem}
\begin{table}[h]
	\centering
	\resizebox{\textwidth}{!}{%
		\begin{tabular}{ccccc}
			\toprule
			\textbf{Modello per}          & \textbf{Dimensione}   & \textbf{Parametro}  & \textbf{Migliore}                                  & \textbf{Distribuzione}                             \\
			\textbf{la popolazione}       & \textbf{del campione} & \textbf{da stimare} & \textbf{stimatore}                                 & \textbf{campionaria}                               \\
			\midrule
			$X \sim N(\mu, \sigma^2)$     & qualsiasi             & $\mu$               & $\bar{X} = \frac{1}{n} \sum X_i$                   & $N\left(\mu, \frac{\sigma^2}{n}\right)$            \\
			\addlinespace
			$X \sim f(\mu, \sigma^2)$     & $n$ grande            & $\mu$               & $\bar{X} = \frac{1}{n} \sum X_i$                   & $N\left(\mu, \frac{\sigma^2}{n}\right)$            \\
			\addlinespace
			$X \sim Ber(\theta)$          & $n$ grande            & $\theta$            & $\hat{\theta} = \frac{1}{n} \sum X_i$              & $N\left(\theta, \frac{\theta(1-\theta)}{n}\right)$ \\
			\addlinespace
			$X \sim \mathcal{P}(\lambda)$ & qualsiasi             & $\lambda$           & $\bar{X} = \frac{1}{n} \sum X_i$                   & $N\left(\lambda, \frac{\lambda}{n}\right)$         \\
			\addlinespace
			$X \sim N(\mu, \sigma^2)$     & qualsiasi             & $\sigma^2$          & $\bar{S}^2 = \frac{1}{n-1} \sum (X_i - \bar{X})^2$ & $\frac{(n-1)S^2}{\sigma^2} \sim \chi^2_{n-1}$      \\
			\bottomrule
		\end{tabular}%
	}
\end{table}
...
Sappiamo che con la stima puntuale possiamo trovare uno stimatore per un parametro ignoto $\theta$ a cui possiamo attribuire un valore numerico (stima). \\ Un "buon"
stimatore assume valori vicini al vero valore del parametro $\theta$, ma la probabilità che lo stimatore assuma valore esattamente uguale al parametro è pari a zero. Quindi, oltre
alla stima puntuale, si riporta un \textcolor{red}{intervallo di confidenza}, ossia un intervallo di valori che ci aspettiamo contenga, con un certo \textcolor{red}{livello di confidenza},
il vero valore del parametro.
\subsubsection{Costruire intervalli di confidenza per $n$ piccolo: Media}
\begin{theorem}{Stima Intervallare della Media con $\sigma^2$ nota (Normale Standard)}
	SSia:
	\begin{itemize}
		\item Popolazione Normale
		\item Varianza nota
	\end{itemize}
	\begin{align*}
		\overline{x}-z_{1-\frac{\alpha}{2}}\cdot \frac{\sigma}{\sqrt{n}} \textcolor{cyan}{\; < \mu < \;} \overline{x}+z_{1-\frac{\alpha}{2}}\cdot \frac{\sigma}{\sqrt{n}}
	\end{align*}
	L'ampiezza intervallare $A$ si misura come segue:
	\begin{align*}
		A = \left(2\cdot z_{1-\frac{\alpha}{2}}\right) \cdot \frac{\sigma}{\sqrt{n}}
	\end{align*}
\end{theorem}
\begin{esercizio}[Esercizio 1]
	Nell'ambito della selezione del personale di una grande azienda, viene richiesto lo svolgimento di un esercizio al computer per il quale
	viene rilevato il tempo di esecuzione. I tempi osservati (in secondi) dei 24 concorrenti sono:
	\begin{align*}
		345, 389, 363, 417, 476, 334, 417, 427 \\
		269, 370, 289, 336, 305, 285, 294, 319 \\
		370, 338, 297, 328, 414, 257, 284, 410
	\end{align*}
	ammettendo che il tempo di esecuzione dell'esercizio abbia distribuzione normale con deviazione standard 50, \textcolor{red}
	{determinare l'intervallo di confidenza al 95$\%$ per la media della popolazione.}
	\begin{center}
		\textbf{Soluzione}
	\end{center}
	Devo trovare la stima intervallare per la media $\mu$ della popolazione e ho i seguenti dati:
	\begin{enumerate}
		\item Campione $n=24$
		\item Varianza nota
		\item Popolazione Normale
		\item Intervallo di confidenza $95\%$
	\end{enumerate}
	quindi uso:
	\begin{align*}
		\left(\overline{x}-z_{1-\frac{\alpha}{2}}\cdot \frac{\sigma}{\sqrt{n}} \quad , \quad \overline{x}+z_{1-\frac{\alpha}{2}}\cdot \frac{\sigma}{\sqrt{n}}\right)
	\end{align*}
	\begin{enumerate}
		\item \textcolor{red}{Trovo $\alpha$}
		      \begin{align*}
			      \alpha = 1 - 0.95 = 0.05
		      \end{align*}
		\item \textcolor{red}{Trovo $1-\frac{\alpha}{2}$}
		      \begin{align*}
			      1-\frac{\alpha}{2} & = 1 - \frac{0.05}{2} \\
			                         & = 0.975
		      \end{align*}
		\item \textcolor{red}{Trovo $\pm z_{1- \frac{\alpha}{2}}$}
		      \begin{align*}
			      z_{0.975} & = ?                                            \\
			                & \text{guardo la tavola della Normale Standard} \\
			                & = 1.96
		      \end{align*}
		      I valori sono
		      \begin{align*}
			      -z_{1- \alpha/2} = -1.96 \quad \quad z_{1- \alpha/2} = 1.96
		      \end{align*}
		      \begin{center}
			      \makebox[\textwidth][c]{%
				      \includegraphics[width=0.5\linewidth]{es-1.png}%
			      }
		      \end{center}
		\item \textcolor{red}{Calcolo intervallo} \\
		      L'esercizio non mi ha fornito la media campionaria, quindi la calcolo
		      \begin{align*}
			      \overline{x} = \frac{\sum x_i}{n} = 347.21
		      \end{align*}
		      allora
		      \begin{align*}
			      \left(\overline{x}-z_{1-\frac{\alpha}{2}}\cdot \frac{\sigma}{\sqrt{n}} \quad \right. & , \left. \quad \overline{x}+z_{1-\frac{\alpha}{2}}\cdot \frac{\sigma}{\sqrt{n}}\right) \\
			      \left(347.21 - 1.96 \cdot \frac{50}{\sqrt{24}} \quad \right.                         & , \left. \quad 347.21 + 1.96 \cdot \frac{50}{\sqrt{24}} \right)                        \\
			      \left(327.21 \quad \right.                                                           & , \left. \quad 367.21 \right) \quad \textcolor{green}{\text{Risposta corretta}}
		      \end{align*}
	\end{enumerate}
\end{esercizio}

\begin{theorem}{Stima Intervallare della Media con $\sigma^2$ ignota ($T-$Student)}
	SSia:
	\begin{itemize}
		\item Popolazione Normale
		\item Varianza \textcolor{red}{non} nota
	\end{itemize}
	\begin{align*}
		\overline{x}-t_{1-\frac{\alpha}{2}}\cdot \frac{s}{\sqrt{n}} \textcolor{cyan}{\; < \mu < \;} \overline{x}+t_{1-\frac{\alpha}{2}}\cdot \frac{s}{\sqrt{n}}
	\end{align*}
	L'ampiezza intervallare $A$ si misura come segue:
	\begin{align*}
		A = \left(2\cdot t_{1-\frac{\alpha}{2}}\right) \cdot \frac{s}{\sqrt{n}}
	\end{align*}
\end{theorem}
\begin{esercizio}[Esercizio 2]
	Il produttore di una certa marca di sigarette desidera controllare il quantitativo medio di catrame in esse contenuto.
	Viene osservato un  campione casuale di 25 sigarette:
	\begin{align*}
		11.1, 10.5, 11.4, 10.7, 11.4, \\
		10.8, 11.2, 10.3, 11.6, 10.6, \\
		12.7, 10.6, 11.7, 10.6, 10.7, \\
		12.2, 10.7, 11.0, 10.5, 11.0, \\
		10.9, 10.7, 10.1, 11.3, 10.5
	\end{align*}
	assumendo che la popolazione sia normale, \textcolor{red}
	{determinare l'intervallo di confidenza per $\mu$ al 99$\%$.}
	\begin{center}
		\textbf{Soluzione}
	\end{center}
	Devo trovare la stima intervallare per la media $\mu$ della popolazione e ho i seguenti dati:
	\begin{enumerate}
		\item Campione $n=25$
		\item Varianza non nota
		\item Popolazione Normale
		\item Intervallo di confidenza $99\%$
	\end{enumerate}
	quindi uso:
	\begin{align*}
		\left(\overline{x}-t_{1-\frac{\alpha}{2}}\cdot \frac{s}{\sqrt{n}} \quad , \quad \overline{x}+t_{t-\frac{\alpha}{2}}\cdot \frac{s}{\sqrt{n}}\right)
	\end{align*}
	\begin{enumerate}
		\item \textcolor{red}{Trovo $\alpha$}
		      \begin{align*}
		...
		\item \textcolor{red}{Trovo $1-\frac{\alpha}{2}$}
		      \begin{align*}
			      1-\frac{\alpha}{2} & = 1 - \frac{0.01}{2} \\
			                         & = 0.995
		      \end{align*}
		\item \textcolor{red}{Trovo $\pm t_{1- \frac{\alpha}{2}}$}
		      \begin{align*}
			      t_{0.995} & = ?                                                                          \\
			                & \text{guardo la tavola della $T-$Student con $n-1=25-1=24$ gradi di libertà} \\
			                & = 2.797
		      \end{align*}
		      I valori sono
		      \begin{align*}
			      -t_{1- \alpha/2} = -2.797 \quad \quad t_{1- \alpha/2} = 2.797
		      \end{align*}
		      \begin{center}
			      \makebox[\textwidth][c]{%
				      \includegraphics[width=0.5\linewidth]{es-2.png}%
			      }
		      \end{center}
		\item \textcolor{red}{Calcolo intervallo} \\
		      L'esercizio non mi ha fornito la media campionaria, quindi la calcolo
		      \begin{align*}
			      \overline{x} = \frac{\sum x_i}{n} = 11
		      \end{align*}
		      L'esercizio non mi ha fornito la varianza campionaria, quindi la calcolo
		      \begin{align*}
			      s^2 = \frac{\sum \left(x_i - \overline{x}\right)^2}{n-1} = 0.357
		      \end{align*}
		      quindi
		      \begin{align*}
			      s = \sqrt{0.357}
		      ...
		      allora
		      \begin{align*}
			      \left(\overline{x}-t_{1-\frac{\alpha}{2}}\cdot \frac{s}{\sqrt{n}} \quad \right. & , \left. \quad \overline{x}+t_{1-\frac{\alpha}{2}}\cdot \frac{s}{\sqrt{n}}\right) \\
			      \left(11 - 2.797 \cdot \frac{\sqrt{0.357}}{\sqrt{25}} \quad \right.             & , \left. \quad 11 + 2.797 \cdot \frac{\sqrt{0.357}}{\sqrt{25}} \right)            \\
			      \left(10.67 \quad \right.                                                       & , \left. \quad 11.33 \right) \quad \textcolor{green}{\text{Risposta corretta}}
		      \end{align*}
	\end{enumerate}
\end{esercizio}

\subsubsection{Costruire intervalli di confidenza per $n$ piccolo: Varianza}
\begin{theorem}{Stima Intervallare della Varianza ($\chi^2_{\nu=n-1}$ Chi-Quadro)}
	SSia:
	\begin{itemize}
		\item Popolazione Normale
	\end{itemize}
	\begin{align*}
		\frac{(n-1)s^2}{\chi^2_{1- \frac{\alpha}{2}}} \textcolor{cyan}{\; < \sigma^2 < \;} \frac{(n-1)s^2}{\chi^2_{\frac{\alpha}{2}}}
	\end{align*}
	L'ampiezza intervallare $A$ si misura come segue:
	\begin{align*}
		A = \left(\frac{(n-1)s^2}{\chi^2_{\frac{\alpha}{2}}}\right) - \left(\frac{(n-1)s^2}{\chi^2_{1- \frac{\alpha}{2}}}\right)
	\end{align*}
\end{theorem}
\begin{esercizio}[Esercizio 3]
	Un'azienda produttrice di orologi di precisione dichiara che questi sono soggetti a un'oscillazione massima di 1 secondo al mese. Viene
	osservato l'errore mensile di 20 orologi e si trova $s^2=0.14$.
	assumendo che la popolazione sia normale, \textcolor{red}
	{determinare l'intervallo di confidenza per $\sigma^2$ al 95$\%$.}
	\begin{center}
		\textbf{Soluzione}
	\end{center}
	Devo trovare la stima intervallare per la media $\sigma^2$ della popolazione e ho i seguenti dati:
	\begin{enumerate}
		\item Campione $n=20$
		\item Popolazione Normale
		\item Intervallo di confidenza $95\%$
	...
	quindi uso:
	\begin{align*}
		\left(\frac{(n-1)s^2}{\chi^2_{1-\alpha /2}} \quad , \quad \frac{(n-1)s^2}{\chi^2_{\alpha /2}}\right)
	\end{align*}
	\begin{enumerate}
		\item \textcolor{red}{Trovo $\alpha$}
		      \begin{align*}
			      \alpha = 1 - 0.95 = 0.05
		      \end{align*}
		\item \textcolor{red}{Trovo $1-\frac{\alpha}{2}, \alpha/2$}
		      \begin{align*}
			      1-\frac{\alpha}{2} & = 1 - \frac{0.05}{2} \\
			                         & = 0.975
		      \end{align*}
		      \begin{align*}
			      \frac{\alpha}{2} & = \frac{0.05}{2} \\
			                       & = 0.025
		      \end{align*}
		\item \textcolor{red}{Trovo $\chi^2_{1-\alpha /2} \,$,$\, \chi^2_{\alpha /2}$}
		      \begin{align*}
			      \chi^2_{0.975} & = ?                                                                         \\
			                     & \text{guardo la tavola della Chi-Quadro con $n-1=20-1=19$ gradi di libertà} \\
			                     & = 32.85
		      \end{align*}
		      \begin{center}
			      \makebox[\textwidth][c]{%
				      \includegraphics[width=0.5\linewidth]{es-3a.png}%
			      }
		      \end{center}
		      \begin{align*}
			      \chi^2_{0.025} & = ?                                                                         \\
		      ...
		      \begin{center}
			      \makebox[\textwidth][c]{%
				      \includegraphics[width=0.5\linewidth]{es-3b.png}%
			      }
		      \end{center}
		      I valori sono
		      \begin{align*}
			      \chi^2_{0.975} = 32.85  \quad \quad \chi^2_{0.025} = 8.91
		      \end{align*}
		\item \textcolor{red}{Calcolo intervallo}
		      \begin{align*}
			      \left(\frac{(n-1)s^2}{\chi^2_{1-\alpha /2}} \quad \right. & , \left. \quad \frac{(n-1)s^2}{\chi^2_{\alpha /2}} \right)                    \\
			      \left(\frac{(20-1)0.14}{32.85} \quad \right.              & , \left. \quad \frac{(20-1)0.14}{8.91} \right)                                \\
			      \left(\frac{19 \cdot 0.14}{32.85} \quad \right.           & , \left. \quad \frac{19 \cdot 0.14}{8.91} \right)                             \\
			      \left(0.08 \quad \right.                                  & , \left. \quad 0.30 \right) \quad \textcolor{green}{\text{Risposta corretta}}
		      \end{align*}
	\end{enumerate}
\end{esercizio}

\subsubsection{Costruire intervalli di confidenza per $n$ grande: Media}
\begin{theorem}{Stima Intervallare della Media per $n$ grande (Normale Standard)}
	SSia:
	\begin{itemize}
		\item Popolazione \textcolor{red}{qualsiasi}
	\end{itemize}
	\begin{align*}
		\overline{x}-z_{1-\frac{\alpha}{2}}\cdot \frac{s}{\sqrt{n}} \textcolor{cyan}{\; < \mu < \;} \overline{x}+z_{1-\frac{\alpha}{2}}\cdot \frac{s}{\sqrt{n}}
	\end{align*}
	L'ampiezza intervallare $A$ si misura come segue:
	\begin{align*}
		A = \left(2\cdot z_{1-\frac{\alpha}{2}}\right) \cdot \frac{s}{\sqrt{n}}
	\end{align*}
\end{theorem}
\begin{esercizio}[Esercizio 4]
	Nel processo di controllo del peso delle confezioni di pasta da 500 grammi, un pastificio considera non conformi
	i pesi al di sotto di 485 grammi o al di sopra di 515 grammi. Per valutare l'efficienza di una confezionatrice,
	l'azienda esamina un campione di 830 confezioni e trova che 42 di esse sono non conformi. \textcolor{red}{Determinare l'intervallo di confidenza al 95$\%$
		per $\theta$}.
	\begin{center}
		\textbf{Soluzione}
	\end{center}
	...
	\begin{enumerate}
		\item Campione $n=830$
		\item Popolazione Bernoullina
		\item Intervallo di confidenza $95\%$
	\end{enumerate}
	quindi uso:
	\begin{align*}
		\left(\overline{x}-z_{1-\frac{\alpha}{2}}\cdot \frac{s}{\sqrt{n}} \quad , \quad \overline{x}+z_{t-\frac{\alpha}{2}}\cdot \frac{s}{\sqrt{n}}\right)
	\end{align*}
	\begin{enumerate}
		\item \textcolor{red}{Trovo $\alpha$}
		      \begin{align*}
		...
		\item \textcolor{red}{Trovo $\pm z_{1- \frac{\alpha}{2}}$} \\
		      ...
		      \end{center}
		\item \textcolor{red}{Calcolo intervallo} \\
		      So che la popolazione è Bernoulliana e che la sua varianza campionaria è $s^2=\theta(1-\theta)$, quindi questa formula
		      \begin{align*}
			      \left(\overline{x}-z_{1-\frac{\alpha}{2}}\cdot \frac{s}{\sqrt{n}} \quad , \quad \overline{x}+z_{t-\frac{\alpha}{2}}\cdot \frac{s}{\sqrt{n}}\right)
		      \end{align*}
		      diventa
		      \begin{align*}
			      \left(\overline{\theta}-z_{1-\frac{\alpha}{2}}\cdot \frac{\sqrt{\overline{\theta}(1-\overline{\theta})}}{\sqrt{n}} \quad ,
			      \quad \overline{\theta}+z_{t-\frac{\alpha}{2}}\cdot \frac{\sqrt{\overline{\theta}(1-\overline{\theta})}}{\sqrt{n}}\right)
		      \end{align*}

		      L'esercizio non mi ha fornito la media campionaria, quindi la calcolo
		      \begin{align*}
			      \overline{\theta} = \frac{\sum x_i}{n} = \frac{n\text{ successi}}{\text{campione}} = \frac{42}{830} = 0.051
		      \end{align*}
		      allora
		      \begin{align*}
			      \left(\overline{\theta}-z_{1-\frac{\alpha}{2}}\cdot \frac{\sqrt{\overline{\theta}(1-\overline{\theta})}}{\sqrt{n}} \quad \right. & , \left. \quad \overline{\theta}+z_{1-\frac{\alpha}{2}}\cdot \frac{\sqrt{\overline{\theta}(1-\overline{\theta})}}{\sqrt{n}}\right) \\
			      \left(0.05 - 1.96 \cdot \frac{\sqrt{{0.05(1-0.05)}}}{\sqrt{830}} \quad \right.                                                   & , \left. \quad 0.05 + 1.96 \cdot \frac{\sqrt{{0.05(1-0.05)}}}{\sqrt{830}} \right)                                                  \\
			      \left(0.036 \quad \right.                                                                                                        & , \left. \quad 0.066 \right) \quad \textcolor{green}{\text{Risposta corretta}}
		      \end{align*}
	\end{enumerate}
\end{esercizio}
\newpage
\begin{center}
	\makebox[\textwidth][c]{%
		\includegraphics[width=1.7\linewidth]{riassunto-stima-intervallare.png}%
	}
\end{center}
\newpage
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Il test d'ipotesi è una procedura che permette di decidere, sulla base di un campione, se c'è abbastanza evidenza per mettere in discussione un'affermazione fatta sulla popolazione.
\begin{theorem}{Ipotesi nulla e Ipotesi alternativa}
	LL'ipotesi sottoposta a verifica viene chiamata \textcolor{red}{ipotesi nulla} (considerata vera fino a prova contraria)
	\begin{align*}
		H_0: \theta = \theta_0
	\end{align*}
	e l'ipotesi che la contraddice, viene denominata \textcolor{red}{ipotesi alternativa} $H_1$.
\end{theorem}
\begin{osservazioni}[Obbiettivo]
	L'obbiettivo è effettuare un \textcolor{red}{test d'ipotesi} per:
	\begin{itemize}
		\item rifiutare $H_0$
		\item non rifiutare $H_0$ (in questo caso non si rifiuta nè si accetta l'ipotesi)
		\item accettare $H_0$
	\end{itemize}
\end{osservazioni}
\subsubsection{Sistema di ipotesi}
\begin{theorem}{Ipotesi alternativa unilaterale destra}
	FFissato un valore del parametro $\theta_0$, si ha:
	\begin{align*}
		\begin{cases}
			H_0: \theta = \theta_0 \quad \text{ipotesi nulla} \\
			H_1 : \theta > \theta_0 \quad \text{ipotesi alternativa unilaterale destra}
		\end{cases}
	\end{align*}
	\begin{center}
		\makebox[\textwidth][c]{%
			\includegraphics[width=0.7\linewidth]{alt-dx.png}%
		}
	\end{center}
	Se il campione estratto proviene:
	\begin{itemize}
		\item da $\mathcal{A}$, si accetta $H_0$
		\item da $\mathcal{R}$, si rifiuta $H_0$ (per ogni campione $> c$, si rifiuta l'ipotesi nulla)
	\end{itemize}
\end{theorem}
\begin{theorem}{Ipotesi alternativa unilaterale sinistra}
	FFissato un valore del parametro $\theta_0$, si ha:
	\begin{align*}
		\begin{cases}
			H_0: \theta = \theta_0 \quad \text{ipotesi nulla} \\
			H_1 : \theta < \theta_0 \quad \text{ipotesi alternativa unilaterale sinistra}
		\end{cases}
	\end{align*}
	\begin{center}
		\makebox[\textwidth][c]{%
			\includegraphics[width=0.7\linewidth]{alt-sx.png}%
		}
	\end{center}
	Se il campione estratto proviene:
	\begin{itemize}
	...
	\end{itemize}
\end{theorem}
\begin{theorem}{Ipotesi alternativa bidirezionale}
	FFissato un valore del parametro $\theta_0$, si ha:
	\begin{align*}
		\begin{cases}
			H_0: \theta = \theta_0 \quad \text{ipotesi nulla} \\
			H_1 : \theta \neq \theta_0 \quad \text{ipotesi alternativa bilaterale}
		\end{cases}
	\end{align*}
	\begin{center}
		\makebox[\textwidth][c]{%
			\includegraphics[width=0.7\linewidth]{alt-bi.png}%
		}
	\end{center}
	Se il campione estratto proviene:
	\begin{itemize}
		\item da $\mathcal{A}$, si accetta $H_0$
		\item da $\mathcal{R}$, si rifiuta $H_0$ (per ogni campione $> c$ e per ogni campione $< c$, si rifiuta l'ipotesi nulla)
	\end{itemize}
\end{theorem}

\subsubsection{Errori del Test}
\begin{itemize}
	\item \textcolor{red}{Errore di I tipo:} rifiutare $H_0$ quando vera (errore più grave)
	\item \textcolor{red}{Errore di II tipo:} accettare $H_0$ quando falsa
\end{itemize}
\begin{esempio}[Esempio]
	Nel contesto giuridico, si assume che l'imputato è innocente fino a prova contraria
	\begin{align*}
		H_0: \text{ imputato innocente}
	\end{align*}
	Quindi:
	\begin{itemize}
		\item Errore di I tipo: l'imputato viene dichiarato colpevole quando in realtà è innocente (questo errore è considerato più grave perchè un fratm ingiustamente ingabbiat' insomma, gli rovini
		      la vita)
		\item Errore di II tipo: l'imputato viene dichiarato innocente quando in realtà è colpevole
	\end{itemize}
\end{esempio}
\begin{theorem}{Probabilità di commettere l'errore}
	S
	\begin{itemize}
		\item \textcolor{red}{Probabilità di commettere l'errore I (livello di significatività):} indicato con $\alpha(\theta)$
		\item \textcolor{red}{Probabilità di commettere l'errore II:} indicato con $\beta(\theta)$
		\item \textcolor{red}{Probabilità di rifiutare $H_0$ quando è falsa\footnote{
				      ovvero la probabilità di dimostrare che $H_0$ è davvero falsa.
			      }(Potenza del test):} indicato con $\pi(\theta)= 1-\beta(\theta)$
	\end{itemize}
\end{theorem}

\subsubsection{Passi da seguire nella verifica delle ipotesi}
\begin{enumerate}
	\item \textcolor{red}{Definire $H_0$ e $H_1$}
	\item \textcolor{red}{Segliere la statistica test (media campionaria, varianza campionaria, ecc..)}
	\item \textcolor{red}{Scegliere il livello di significatività $\alpha$}
	\item \textcolor{red}{Definizione del valore critico $c$ e della regione di rifiuto}
	      \begin{align*}
		      c = \begin{cases}
			           & z_{1-\alpha} \quad \text{con ipotesi alternativa destra}       \\
			           & -z_{1-\alpha} \quad \text{con ipotesi alternativa sinistra}    \\
			           & z_{1-\alpha/2} \quad \text{con ipotesi alternativa bilaterale} \\
		          \end{cases}
	      \end{align*}
	\item \textcolor{red}{Estrazione del campione}
	\item \textcolor{red}{Calcolo del valore osservato $z_{oss}$ attraverso la statistica test}
	\item \textcolor{red}{Analizzare se il valore osservato cade nella regione di rifiuto o nella regione di accettazione:}
	      \begin{itemize}
		      \item Se $z_{oss}$ cade nella regione di rifiuto, allora si rifiuta $H_0$.
		      \item Se $z_{oss}$ cade nella regione di accettazione, allora non si rifiuta $H_0$ al livello di significatività fissato.
	      \end{itemize}
\end{enumerate}
...
\begin{esercizio}[Esercizio 1]
	...
	\begin{align*}
		...
		\end{cases}
	\end{align*}
	con $\alpha=0.01$, $\overline{x}=502.9$ , $\sigma^2=35$ e con $n=40$ campioni. \textcolor{red}{Verificare l'ipotesi nulla $H_0$.}
	\begin{center}
		\textbf{Soluzione}
	\end{center}
	\begin{enumerate}
		\item Calcolare valore critico $z_{1-\alpha}$
		      \begin{align*}
			      z_{1-\alpha}=z_{1-0.01}=z_{0.99}=2.326
		      \end{align*}
		\item Definire zona di rifiuto
		      \begin{align*}
			      \mathcal{R}_{\alpha}=\left\{\frac{\overline{x}-\mu_0}{\frac{\sigma}{\sqrt{n}}} > z_{1-\alpha}\right\}=\left\{\frac{\overline{x}-500}{\frac{\sqrt{35}}{\sqrt{40}}} > 2.326 \right\}
		      \end{align*}
		\item Calcolo del valore osservato $z_{oss}$ attraverso la statistica test
		      \begin{align*}
			      z_{oss}=\frac{\overline{x}-500}{\frac{\sqrt{35}}{\sqrt{40}}} = \frac{502.9-500}{\frac{\sqrt{35}}{\sqrt{40}}} = 3.100
		      \end{align*}
		\item Analizzare se $z_{oss}$ cade nella regione di rifiuto o no
		      \begin{align*}
			      3.100 > 2.326 \implies & z_{oss} \text{ cade nella regione di rifiuto} \\ &\text{quindi \textbf{rifiuto} l'ipotesi nulla } H_0.
		      \end{align*}
	\end{enumerate}
\end{esercizio}
...
\begin{esercizio}[Esercizio 2]
	...
	\begin{align*}
		...
		\end{cases}
	\end{align*}
	con $\alpha=0.05$, $\sigma^2=50$ e con i seguenti campioni:
	\begin{align*}
		43.5, 32, 26.4, 37.8, 25, 31, 46.8, 19 \\
		28.5, 35, 23.7, 32.5, 37.9, 36, 31.4
	\end{align*}
	\textcolor{red}{Verificare l'ipotesi nulla $H_0$.}
	\begin{center}
		\textbf{Soluzione}
	\end{center}
	\begin{enumerate}
		...
		\item Definire zona di rifiuto
		      \begin{align*}
			      \mathcal{R}_{\alpha}=\left\{\frac{\overline{x}-\mu_0}{\frac{\sigma}{\sqrt{n}}} > z_{1-\alpha}\right\}=\left\{\frac{\overline{x}-30}{\frac{\sqrt{50}}{\sqrt{15}}} > 1.645 \right\}
		      \end{align*}
		\item Calcolo del valore osservato $z_{oss}$ attraverso la statistica test
		      \begin{align*}
			      z_{oss}=\frac{\overline{x}-30}{\frac{\sqrt{50}}{\sqrt{15}}} =
		      \end{align*}
		      mi manca il valore di $\overline{x}$, lo calcolo:
		      \begin{align*}
			      \overline{x} = \frac{43.5+32+ 26.4+ \dots + 32.5+ 37.9+ 36+ 31.4}{15} = 32.43
		      \end{align*}
		      quindi:
		      \begin{align*}
			      z_{oss}=\frac{\overline{x}-30}{\frac{\sqrt{50}}{\sqrt{15}}} = \frac{32.43-30}{\frac{\sqrt{50}}{\sqrt{15}}} = 1.330
		      \end{align*}
		\item Analizzare se $z_{oss}$ cade nella regione di rifiuto o no
		      \begin{align*}
			      1.330 < 1.645 \implies & z_{oss} \text{ cade nella regione di accettazione\footnotemark} \\
			                             & \text{quindi \textbf{non rifiuto} l'ipotesi nulla } H_0.
		      \end{align*}
		      \footnotetext{perché non cade nella regione di rifiuto, essendo minore del valore critico}
	\end{enumerate}
\end{esercizio}
...
\begin{esercizio}[Esercizio 3]
	...
	\begin{align*}
		...
		\end{cases}
	\end{align*}
	con $\alpha=0.05$, $\sigma^2=4$ e $\overline{x}=9.5$ e $n=5$. \textcolor{red}{Verificare l'ipotesi nulla $H_0$.}
	\begin{center}
		\textbf{Soluzione}
	\end{center}
	\begin{enumerate}
		\item Calcolare il valore critico $|z_{1-\alpha/2}|$
		      \begin{align*}
			      z_{1-\alpha/2}=z_{1-0.05/2}=z_{0.975}=1.960
		      \end{align*}
		      quindi
		      \begin{align*}
			      z_{1-\alpha/2}=1.960 \quad \quad -z_{1-\alpha/2}=-1.960
		      \end{align*}
		\item Definire zona di rifiuto
		      \begin{align*}
			      \mathcal{R}_{\alpha} = \left\{\frac{\overline{x}-\mu_0}{\sigma/\sqrt{n}} > z_{1-\alpha/2} \right\} \cup
			      \left\{\frac{\overline{x}-\mu_0}{\sigma/\sqrt{n}} < -z_{1-\alpha/2}\right\}
		      \end{align*}
		      \begin{align*}
			      \mathcal{R}_{\alpha} = \left\{\frac{\overline{x}-8}{\sqrt{4}/\sqrt{5}} > 1.960 \right\} \cup
			      \left\{\frac{\overline{x}-8}{\sqrt{4}/\sqrt{5}} < -1.960\right\}
		      \end{align*}
		\item Calcolo $z_{oss}$ attraverso la statistica test (media campionaria)
		      \begin{align*}
			      z_{oss} = \frac{\overline{x}-8}{\sqrt{4/5}} = \frac{9.5-8}{\sqrt{4/5}} = 1.677
		      \end{align*}
		\item Analizzo se $z_{oss}$ è cade nella regione di rifiuto o no
		      \begin{align*}
			      1.667 \text{ sta tra -1.960 e +1.960 quindi cade nella regione di accettazione} \\
			      \text{quindi \textbf{non rifiuto} l'ipotesi nulla } H_0.
		      \end{align*}
	\end{enumerate}
	Oppure posso semplicemente dire che:
	\begin{align*}
		\text{se } |z_{oss}| > z_{1- \alpha/2} \implies \text{ rifiuto } H_0
	\end{align*}
	ma ho visto che:
	\begin{align*}
		|z_{oss}| = |1.677| \\
		|1.667| < 1.960 \implies \text{ non rifiuto } H_0
	\end{align*}
\end{esercizio}

\subsubsection{$p-$value}
È un approccio alternativo per decidere se accettare o rifiutare l'ipotesi nulla.
\begin{theorem}{$p-$value}
	IIl \textcolor{red}{$p-$value} è la probabilità che la statistica test ($\overline{x}$ oppure $s^2$) assuma un valore uguale o più estremo (nella direzione dell'ipotesi alternativa) di quello
	osservato $z_{oss}$ quando $H_0$ è vera.
	\begin{align*}
		\begin{cases}
			p-\text{value} < \alpha \implies \text{ Rifiutare } H_0 \\
			p-\text{value} > \alpha \implies \text{ Non Rifiutare } H_0
		\end{cases}
	\end{align*}
\end{theorem}
\begin{esempio}[Esempio $p-$value]
	Siano:
	\begin{align*}
		\begin{cases}
			H_0: \mu = 8 \\
	...
	\end{enumerate}
	\begin{center}
		\textbf{Soluzione con $p-$value}
	\end{center}
	\begin{enumerate}
		\item Calcolo $p-$value:
		      \begin{align*}
			      \textcolor{red}{P(|\overline{x}|> |z_{oss}|)} & = P(|{\overline{x}}|> |z_{oss}|)  \\
			                                                    & = 2 \cdot [1-P(Z \leq |z_{oss}|)] \\
			                                                    & = 2 \cdot [1- \phi(|z_{oss}|)]    \\
			                                                    & = 2 \cdot [1- \phi(|1.677|)]      \\
			                                                    & = 2 \cdot [1- \phi(1.677)]        \\
			                                                    & = 2 \cdot [1- 0.9535] = 0.093
		      \end{align*}
		      quindi la probabilità che la media campionaria (statistica test) sia maggiore del valore osservato (in valore assoluto) è pari a 0.093.
		\item Analizzo se $p-$value è maggiore o minore di $\alpha$
		      \begin{align*}
			      p-\text{value} = 0.093
		      \end{align*}
		      e $0.093 > 0.05 \implies$ \textbf{non rifiutare} $H_0$.
	\end{enumerate}
\end{esempio}

\subsubsection{Test di ipotesi sulla media di una popolazione normale con varianza nota}
\begin{esercizio}[Test unilaterale destro per $\mu$ di una popolazione Normale con $\sigma^2$ noto]
	Il tempo di installazione di un certo hardware ha distribuzione normale di media $\mu$ e $\sigma^2=25$. Un tecnico installa l'hw in 64 pc impiegandoci 42 minuti e $\alpha=0.05$.
	\textcolor{red}{Valutare
		l'ipotesi nulla $H_0$.}
	\begin{align*}
		\begin{cases}
			H_0: \mu = 40 \text{ minuti} \\
			H_1: \mu > 40 \text{ minuti}
		\end{cases}
	\end{align*}
	\begin{center}
		\textbf{Soluzione}
	\end{center}
	\begin{enumerate}
		\item Calcolare il valore critico
		      \begin{align*}
			      z_{1-\alpha}=z_{1-0.05}=z_{0.95}=1.645
		      \end{align*}
		\item Definire la regione di rifiuto
		      \begin{align*}
			      \mathcal{R}_{\alpha}=\left\{\frac{\overline{x}-\mu_0}{\sigma/\sqrt{n}} > z_{1-\alpha} \right\} = \left\{\frac{\overline{x}-40}{\sqrt{25/64}} > 1.645\right\}
		      \end{align*}
		\item Calcolare il valore osservato
		      \begin{align*}
			      z_{oss} = \frac{\overline{x}-40}{\sqrt{25/64}} = \frac{42-40}{\sqrt{25/64}} = 3.2
		      \end{align*}
		\item Analizzare se il valore osservato cade nella regione di rifiuto o no
		      \begin{align*}
			      3.2 > 1.645 \implies \text{ cade nella regione di rifiuto, quindi si rifiuta } H_0.
		      \end{align*}
	\end{enumerate}
	\begin{center}
		\textbf{Soluzione con $p-$value}
	...
	\begin{enumerate}
		\item Calcolo $p-$value:
		      \begin{align*}
			      \textcolor{red}{P(\overline{x} > z_ {oss})} & = 1 - P(Z \leq z_ {oss}) \\
			                                                  & = 1 - \phi(z_{oss})      \\
			                                                  & = 1- \phi(3.2)           \\
			                                                  & = 1- 0.9993 = 0.0007
		      \end{align*}
		\item Analizzo se $p-$value è minore o maggiore di $\alpha$:
		      \begin{align*}
			      p-\text{value} = 0.0007 \\
			      0.0007 < 0.05 \implies \text{\textbf{ rifiutare $H_0$}}
		      \end{align*}
	\end{enumerate}
\end{esercizio}

\begin{esercizio}[Test unilaterale sinistro per $\mu$ di una popolazione Normale con $\sigma^2$ noto]
	Siano i dati $n=32, \sigma=0.0076, \overline{x}=1.247$ e $\alpha=0.05$.
	\textcolor{red}{Valutare
		l'ipotesi nulla $H_0$.}
	\begin{align*}
		\begin{cases}
	...
	\begin{enumerate}
		...
		\item Definire la regione di rifiuto
		      \begin{align*}
			      \mathcal{R}_{\alpha}=\left\{\frac{\overline{x}-\mu_0}{\sigma/\sqrt{n}} < -z_{1-\alpha} \right\} = \left\{\frac{\overline{x}-1.25}{0.0076/\sqrt{32}} < -1.645\right\}
		      \end{align*}
		\item Calcolare il valore osservato
		      \begin{align*}
			      z_{oss} = \frac{\overline{x}-1.25}{0.0076/\sqrt{32}} = \frac{1.247-1.25}{0.0076/\sqrt{32}} = -2.233
		      \end{align*}
		\item Analizzare se il valore osservato cade nella regione di rifiuto o no
		      \begin{align*}
			      -2.233 < -1.645 \implies \text{ cade nella regione di rifiuto, quindi si rifiuta } H_0.
		      \end{align*}
	\end{enumerate}
	\begin{center}
		\textbf{Soluzione con $p-$value}
	...
	\begin{enumerate}
		\item Calcolo $p-$value:
		      \begin{align*}
			      \textcolor{red}{P(\overline{x} < z_ {oss})} & = \phi(z_{oss})       \\
			                                                  & = \phi(-2.233)        \\
			                                                  & = 1- \phi(2.233)      \\
			                                                  & = 1 - 0.9871 = 0.0129
		      \end{align*}
		\item Analizzo se $p-$value è minore o maggiore di $\alpha$:
		      \begin{align*}
			      p-\text{value} = 0.0129 \\
			      0.0129 < 0.05 \implies \text{\textbf{ rifiutare $H_0$}}
		      \end{align*}
	\end{enumerate}
\end{esercizio}

\begin{esercizio}[Test bilaterale per $\mu$ di una popolazione Normale con $\sigma^2$ noto]
	Siano i dati $n=32, \sigma=0.0076, \overline{x}=1.247$ e $\alpha=0.05$.
	\textcolor{red}{Valutare
		l'ipotesi nulla $H_0$.}
	\begin{align*}
	...
	\begin{enumerate}
		...
		\item Definire la regione di rifiuto
		      \begin{align*}
			      \mathcal{R}_{\alpha}=\left\{\frac{\overline{x}-\mu_0}{\sigma/\sqrt{n}} < -z_{1-\alpha/2} \right\} \cup \left\{\frac{\overline{x}-\mu_0}{\sigma/\sqrt{n}} > z_{1-\alpha/2} \right\}
		      \end{align*}
		      \begin{align*}
			      \mathcal{R}_{\alpha}=\left\{\frac{\overline{x}-1.25}{0.0076/\sqrt{32}} < -1.96 \right\} \cup \left\{\frac{\overline{x}-1.25}{0.0076/\sqrt{32}} > 1.96 \right\}
		      \end{align*}
		\item Calcolare il valore osservato
		      \begin{align*}
			      z_{oss} = \frac{\overline{x}-1.25}{0.0076/\sqrt{32}} = \frac{1.247-1.25}{0.0076/\sqrt{32}} = -2.233
		...
		\item Analizzare se il valore osservato cade nella regione di rifiuto o no
		      \begin{align*}
			      -2.233 < -1.96 \implies \text{ cade nella regione di rifiuto, quindi si rifiuta } H_0.
		      \end{align*}
	\end{enumerate}
	\begin{center}
		\textbf{Soluzione con $p-$value}
	...
	\begin{enumerate}
		\item Calcolo $p-$value:
		      \begin{align*}
			      \textcolor{red}{P(|\overline{x}| > |z_ {oss}|)}
			       & = 2 \cdot \left[1- P(Z \leq |z_ {oss}|)\right] \\
			       & = 2 \cdot [1- \phi(|-2.233|)]                  \\
			       & = 2 \cdot [1- \phi(2.233)]                     \\
			       & = 2 \cdot [1- 0.9871] = 0.0258
		      \end{align*}
		\item Analizzo se $p-$value è minore o maggiore di $\alpha$:
		      \begin{align*}
			      p-\text{value} = 0.0258 \\
			      0.0258 < 0.05 \implies \text{ \textbf{rifiutare $H_0$}}
		      \end{align*}
	\end{enumerate}
\end{esercizio}
\newpage

\subsubsection{Test di ipotesi sulla media di una popolazione normale con varianza non nota}
Stessa procedura del capitoletto 8.6.5 ma al posto di $\sigma^2$ inserisco $s^2$ (varianza campionaria)
e uso distribuzione $T-$Student e non la Normale Standard.

\begin{esercizio}[Test unilaterale destro per $\mu$ di una popolazione Normale con $\sigma^2$ ignota]

\end{esercizio}


\begin{esercizio}[Test unilaterale sinistro per $\mu$ di una popolazione Normale con $\sigma^2$ ignota]
	Siano $n=12, \overline{x}=47.28, s=2.73$ e $\alpha=0.01$. \textcolor{red}{Verifica l'ipotesi nulla $H_0$}
	\begin{align*}
		\begin{cases}
			H_0: \mu \geq 50 \\
			H_1: \mu < 50
	...
	\end{center}
	\begin{enumerate}
		\item Trovo il valore critico
		\begin{align*}
			-t_{1-\alpha, n-1} = -t_{1-0.01, 11} = -t_{0.99, 11} = -2.718
		\end{align*}
		\item Definisco la regione di rifiuto 
		\begin{align*}
			\mathcal{R}_{\alpha}=\left\{\frac{\overline{x}-\mu_0}{s/\sqrt{n}}< -t_{1-\alpha/2} \right\}=
			\left\{\frac{\overline{x}-50}{2.73/\sqrt{12}}< -2.718 \right\}
		\end{align*}
		\item Calcolo il valore osservato
		\begin{align*}
			t_{oss} = \frac{\overline{x}-50}{2.73/\sqrt{12}} = \frac{47.28-50}{2.73/\sqrt{12}} = -3.451
		\end{align*}
		\item Analizzo se il valore osservato cade nella zona di rifiuto oppure no
		\begin{align*}
			-3.451 < -2.718 \implies \text{ cade nella zona di rifiuto, quindi \textbf{rifiuto} } H_0 \textcolor{green}{\scalebox{1.6}{ \checkmark}}
		\end{align*}
	\end{enumerate}
	\begin{center}
		\textbf{Soluzione con $p-$value}
	\end{center}
	\begin{enumerate}
		\item Calcolo il $p-$value
		\begin{align*}
			\textcolor{red}{P(T < t_{oss})} &= \phi(t_{oss}) \\
			&= \phi(-3.451) \\
			&= 1 - \phi(3.451) \\
			& \text{ il valore di }  \phi(3.451) \text{ con } n-1=12-1=11 \text{ gradi di libertà} \\
			&= 1 - 0.997 = 0.003
		\end{align*}
		\item Analizzo se $p-$value è maggiore o no di $\alpha$
		\begin{align*}
			0.003 < 0.01 \implies \text{ \textbf{rifiuto $H_0$}} \textcolor{green}{\scalebox{1.6}{ \checkmark}}
		\end{align*}
	\end{enumerate}
\end{esercizio}

\begin{esercizio}[Test bilaterale per $\mu$ di una popolazione Normale con $\sigma^2$ ignota]
	Il consumo di acqua (litri al giorno) per abitazione ha distribuzione Normale con media $\mu$ e varianza $\sigma^2$.
	In un campione di $n=16$ abitazioni si osserva $\overline{x}=353.8$ litri/giorno, $s=21.85$ litri/giorno con $\alpha=0.10$.
	\textcolor{red}{Verifica l'ipotesi nulla $H_0$}.
	\begin{align*}
		\begin{cases}
			H_0: \mu = 350 \\
			H_1: \mu \neq 350
	...
	\end{center}
	\begin{enumerate}
		\item Trovo il valore critico con $n-1$ gradi di libertà
		      \begin{align*}
			      t_{1-\alpha/2, n-1}=t_{1-0.10/2, 15}=t_{0.95, 15}=1.753
		      \end{align*}
		      quindi
		      \begin{align*}
			      -t_{1-\alpha/2}=-1.753 \quad \quad t_{1-\alpha/2}=1.753
		      \end{align*}
		\item Definisco la regione di rifiuto
		      \begin{align*}
			      \mathcal{R}_{\alpha}=\left\{\frac{\overline{x}-\mu_0}{s/\sqrt{n}}< -t_{1-\alpha/2} \right\} \cup
			      \left\{\frac{\overline{x}-\mu_0}{s/\sqrt{n}} > t_{1-\alpha/2} \right\}
		      \end{align*}
		      \begin{align*}
			      \mathcal{R}_{\alpha}=\left\{\frac{\overline{x}-350}{21.85/\sqrt{16}}< -1.753 \right\} \cup
			      \left\{\frac{\overline{x}-350}{21.85/\sqrt{16}} > 1.753 \right\}
		      \end{align*}
		\item Calcolo il valore osservato
		      \begin{align*}
			      t_{oss} = \frac{\overline{x}-350}{21.85/\sqrt{16}} = \frac{353.8-350}{21.85/\sqrt{16}} = 0.696
		      \end{align*}
		\item Analizzo se il valore osservato cade nella regione di rifiuto o no
		      \begin{align*}
			      -1.753 < 0.696 < 1.753 \implies \text{ cade nella zona di accettazione} \\
			      \text{quindi \textbf{non rifiuto} } H_0 \textcolor{green}{\scalebox{1.6}{ \checkmark}}
		      \end{align*}
	\end{enumerate}
	\begin{center}
		\textbf{Soluzione con $p-$value}
	...
	\begin{enumerate}
		\item Calcolo il $p-$value
		      \begin{align*}
			      \textcolor{red}{P(|T|>|t_{oss}|)} & = 2 \cdot [1 - P(T \leq |t_{oss}|)]                                                \\
			                                        & = 2 \cdot [1 - \phi(|T_{oss}|)]                                                    \\
			                                        & = 2 \cdot [1 - \phi(|0.696|)]                                                      \\
			                                        & = 2 \cdot [1 - \phi(0.696)]                                                        \\
			                                        & \text{il valore di } \phi(0.696) \text{ con } n-1=16-1=15 \text{ gradi di libertà} \\
			                                        & \text{ sta tra $p=0.70$ e $p=0.80$ quindi } (0.70+0.80)/2=0.75                     \\
			                                        & = 2 \cdot [1 - 0.75] = 0.5
		      \end{align*}
		      \begin{center}
			      \makebox[\textwidth][c]{%
				      \includegraphics[width=0.5\linewidth]{es-3-bil-sigma-ignoto.png}%
			      }
		      \end{center}
		\item Analizzo se il $p-$value è maggiore o minore di $\alpha$
		\begin{align*}
			& p-\text{value}=0.5 \\
			& 0.5 > 0.10 \implies \text{ \textbf{non rifiuto} } H_0 \textcolor{green}{\scalebox{1.6}{ \checkmark}}
		\end{align*}
	\end{enumerate}
\end{esercizio}
\newpage

\subsubsection{Test di ipotesi sulla media di una popolazione qualsisi nel caso di grandi campioni}
Quando il campione è sufficientemente grande, possiamo approssimarlo alla Normale.

\begin{esercizio}[Test bilaterale per $\mu$ di una popolazione qualsiasi con $n$ grande (Normale)]
	slide 61
\end{esercizio}

\begin{esercizio}[Test unilaterale sinistro per $\mu$ di una popolazione qualsiasi con $n$ grande (Normale)]
	Siano $n=60, \overline{x}=9.1, s=4.2$ e $\alpha=0.01$. \textcolor{red}{Verificare l'ipotesi nulla $H_0$.}
	\begin{align*}
		\begin{cases}
			H_0: \mu = 10\\
			H_1: \mu < 10
	...
	\begin{enumerate}
		...
		\item Definisco la regione di rifiuto
		\begin{align*}
			 \mathcal{R}_{\alpha}=\left\{\frac{\overline{x}-\mu_0}{s/\sqrt{n}} < -z_{1-\alpha}\right\} = \left\{\frac{\overline{x}-10}{4.2/ \sqrt{60}} < -2.326 \right\} 
		\end{align*}
		\item Calcolo il valore osservato
		\begin{align*}
			z_{oss}= \frac{\overline{x}-10}{4.2/ \sqrt{60}} = \frac{9.1-10}{4.2/ \sqrt{60}} = -1.660
		\end{align*}
		\item Analizzo se il valore osservato cade nella zona di rifiuto o no
		\begin{align*}
			 -1.660 > -2.326 \implies \text{ cade nella regione di accettazione} \\ \text{quindi \textbf{non rifiuto $H_0$}} \textcolor{green}{\scalebox{1.6}{ \checkmark}}
		\end{align*}
	\end{enumerate}
	\begin{center}
		\textbf{Soluzione con $p-$value}
	...
	\begin{enumerate}
		\item Calcolo $p-$value
		\begin{align*}
			\textcolor{red}{P(\overline{x} < z_{oss})} &= P(Z < z_{oss}) \\
			&= \phi(z_{oss}) \\
			&= \phi(-1.66) \\
			&= 1- \phi(1.66) \\
			&= 1- 0.9515 = 0.0485
		...
		\begin{align*}
			&p-\text{value}=0.0485 \\
			& 0.0485 > 0.01 \implies \text{ \textbf{non rifiutare $H_0$}} \textcolor{green}{\scalebox{1.6}{ \checkmark}}
		\end{align*}
	\end{enumerate}
\end{esercizio}

\begin{osservazioni}[Varianza campionaria - Bernoulli]
	Nel caso di Bernoulli
	\begin{align*}
			s^2=\theta(1-\theta)
	\end{align*}
\end{osservazioni} 

\begin{esercizio}[Test unilaterale destro per $\mu$ di una popolazione qualsiasi con $n$ grande (Bernoulli)]
	Siano $n=300$ pc comprati di cui 9 difettosi e $\alpha=0.05$. \textcolor{red}{Verifica l'ipotesi nulla $H_0$.}
	\begin{align*}
		\begin{cases}
			H_0: \mu \leq 0.02 \\
			H_1: \mu > 0.02
	...
	\begin{enumerate}
		...
		\item Definisco la regione di rifiuto
		\begin{align*}
			 \mathcal{R}_{\alpha}=\left\{\frac{\overline{x}-\mu_0}{s/\sqrt{n}} > z_{1-\alpha} \right\}
		\end{align*}
		\begin{center}
			ma la popolazione è Bernoulliana, quindi:
		\end{center}
		\begin{align*}
			 \mathcal{R}_{\alpha}&=\left\{\frac{\overline{x}-\theta_0}{\frac{\sqrt{\theta_0(1-\theta_0)}}{\sqrt{n}}} > z_{1-\alpha} \right\} \\ 
			 &= \left\{\frac{\overline{x}-\theta_0}{\sqrt{\theta_0(1-\theta_0)/n}} > z_{1-\alpha} \right\} \\
			 &= \left\{\frac{\overline{x}-0.02}{\sqrt{0.02(1-0.02)/300}} > 1.645 \right\}
		\end{align*}
		\item Calcolo il valore osservato
		\begin{align*}
			\text{prima devo calcolare } \overline{x}=\frac{\text{successi}}{n}=\frac{k}{n}=\frac{9}{300}=0.03
		\end{align*}
		\begin{align*}
			z_{oss}= \frac{\overline{x}-0.02}{\sqrt{0.02(1-0.02)/300}} = \frac{0.03-0.02}{\sqrt{0.02(1-0.02)/300}} = 1.237
		\end{align*}
		\item Analizzo se il valore osservato cade nella regione di rifiuto o no
		\begin{align*}
			1.237 < 1.645 \implies \text{ cade nella regione di accettazione} \\ \text{quindi \textbf{non rifiuto $H_0$}} \textcolor{green}{\scalebox{1.6}{ \checkmark}}
		\end{align*}
	\end{enumerate}
	\begin{center}
		\textbf{Soluzione con $p-$value}
	...
	\begin{enumerate}
		...
		\begin{align*}
			&p-\text{value}=0.11 \\
			&0.11 > 0.05 \implies \text{\textbf{non rifiutare $H_0$}} \textcolor{green}{\scalebox{1.6}{ \checkmark}}
		\end{align*}
	\end{enumerate}
\end{esercizio}
\newpage

\subsubsection{Test di ipotesi sulla varianza di una popolazione normale}
\begin{osservazioni}[Varianza campionaria]
	\begin{align*}
		s^2=\frac{\sum(x_i - \overline{x})^2}{n-1}
	\end{align*}
\end{osservazioni}
\begin{esercizio}[Test bilaterale sulla $\sigma^2$ di una popolazione Normale]
	Siano $\sigma^2=0.6, \alpha=0.05$ e 8 campioni:
	\begin{align*}
		22.4, 23.1, 24.5, 24.0, 22.8, 23.6, 24.2, 22.3
	\end{align*}
	\textcolor{red}{Verificare l'ipotesi nulla $H_0$.}
	\begin{align*}
		\begin{cases}
	...
	\begin{enumerate}
		\item Trovo il valore critico
		\begin{align*}
			&\chi^2_{\alpha/2, n-1} = \chi^2_{0.05/2, 7} = \chi^2_{0.025, 7} = 1.69 \\
			&\chi^2_{1-\alpha/2, n-1} = \chi^2_{1-0.05/2, 7} = \chi^2_{0.975, 7} = 16.01
		\end{align*}
		\item Definisco la regione di rifiuto
		\begin{align*}
			 \mathcal{R}_{\alpha} = \left\{\frac{(n-1)s^2}{\sigma^2_0} < \chi^2_{\alpha/2} \right\} \cup \left\{\frac{(n-1)s^2}{\sigma^2_0} > \chi^2_{1-\alpha/2} \right\}
		\end{align*}
		Calcolo media $\overline{x}$ e varianza campionaria $s^2$:
		\begin{align*}
			\overline{x} = \frac{22.4+23.1+24.5+24.0+22.8+23.6+24.2+22.3}{8} = 23.36
		\end{align*}
		\begin{align*}
			s^2 = \frac{(22.4-23.36)^2+ \dots + (22.3-23.36)^2}{7} = 0.70 
		\end{align*}
		quindi
		\begin{align*}
			 \mathcal{R}_{\alpha} = \left\{\frac{(7)s^2}{0.6} < \chi^2_{\alpha/2} \right\} \cup \left\{\frac{(7)s^2}{0.6} > \chi^2_{1-\alpha/2} \right\}
		\end{align*}
		\begin{align*}
			 \mathcal{R}_{\alpha} = \left\{\frac{(7)s^2}{0.6} < 1.69 \right\} \cup \left\{\frac{(7)s^2}{0.6} > 16.01 \right\}
		\end{align*}
		\item Calcolo il valore osservato
		\begin{align*}
			v_{oss} = \frac{(7)s^2}{0.6} = \frac{(7)0.70}{0.6} = 8.17
		\end{align*}
		\item Analizzo se il valore osservato cade nella regione di rifiuto o no
		\begin{align*}
			1.69 < 8.17 < 16.01 \implies \text{ cade nella regione di accettazione} \\ \text{ quindi \textbf{non rifiuto $H_0$}} \textcolor{green}{\scalebox{1.6}{ \checkmark}}
		\end{align*}
	\end{enumerate}
	\begin{center}
		\textbf{Soluzione con $p-$value}
	...
	\begin{enumerate}
		\item Calcolo il $p-$value
		\begin{align*}
			p &= \textcolor{red}{2 \cdot min \left\{P(\chi^2_{n-1} < v_{oss}), P(\chi^2_{n-1} > v_{oss}) \right\}} \\
			&= 2 \cdot min \left\{P(\chi^2_{7} < 8.17), P(\chi^2_{7} > 8.17) \right\} \\
			&= 2 \cdot min \left\{P(\chi^2_{7} < 8.17), 1 - P(\chi^2_{7} \leq 8.17) \right\} \\
			&= 2 \cdot min \left\{0.625, 1 - 0.625 \right\} \\
			&= 2 \cdot min \left\{0.625, 0.375 \right\} \\
			& \text{il minimo è } 0.375 \text{ quindi} \\
			&= 2 \cdot 0.375 = 0.75
		\end{align*}
		\item Analizzo se $p-$value è maggiore o no di $\alpha$
		\begin{align*}
			0.75 > 0.05 = \alpha \implies \text{ \textbf{ non rifiuto $H_0$}} \textcolor{green}{\scalebox{1.6}{ \checkmark}}
		\end{align*}
	\end{enumerate}
\end{esercizio}
\begin{esempio}[Test unilaterale destro sulla $\sigma^2$ di una popolazione Normale]
	slide 55
\end{esempio}
\begin{esempio}[Test unilaterale sinistro sulla $\sigma^2$ di una popolazione Normale]
	slide 56
\end{esempio}
\newpage
\begin{center}
	\makebox[\textwidth][c]{%
		\includegraphics[width=1.5\linewidth]{riassunto-test-ipotesi-n-piccolo.png}%
	}
\end{center}
\begin{center}
	\makebox[\textwidth][c]{%
